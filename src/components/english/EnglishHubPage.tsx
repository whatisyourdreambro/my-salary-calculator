...
import { ENGLISH_HUBS, type EnglishHubId } from "@/lib/englishHubs";

export default function EnglishHubPage({ id }: { id: EnglishHubId }) {
  const hub = ENGLISH_HUBS[id];
  return <EnglishPageShell eyebrow="Moneysalary · English" title={hub.title} description={hub.description} breadcrumbs={[{ name: hub.title, href: hub.path }]}>
