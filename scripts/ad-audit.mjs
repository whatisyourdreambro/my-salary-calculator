...
if (DIFF_MODE) {
  ...
  for (const ln of lines) {
    ...
    if (!inHunk && ln.startsWith("--- ")) {
      oldFile = ln.startsWith("--- a/") ? ln.slice(6) : "";
      meta.push(null);
      continue;
    }
    if (!inHunk && ln.startsWith("+++ ")) {
      // 파일 통째 삭제(+++ /dev/null)는 새 경로가 없으므로 옛 경로로 보고 — 삭제 줄의 광고 소실 검사용
      fileDeleted = ln.startsWith("+++ /dev/null");
      file = ln.startsWith("+++ b/") ? ln.slice(6) : fileDeleted ? oldFile : "";
      meta.push(null);
      continue;
    }
    ...
    else if (ln.startsWith("+") || ln.startsWith(" ")) meta.push({ file, hunk, newNo: newNo++, deleted: fileDeleted });
    else if (ln.startsWith("-")) meta.push({ file, hunk, newNo: 0, deleted: fileDeleted });
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (ln.startsWith("-") && adLineRe.test(ln)) {
      ...
      if (readded) continue;
      if (m.deleted && retiredFiles.has(m.file)) {
        retiredHits.push(`${m.file} (${body.trim().slice(0, 50)})`);
      } else warns.push(`${m.file}: 광고 유닛 삭제 감지, 대체 추가 없음 — 의도 확인 필요`);
    }
  }
