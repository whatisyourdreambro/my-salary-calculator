...
export default function KakaoBonusClient() {
  ...
  const [stockPrice, setStockPrice] = useState(DEFAULT_KAKAO_STOCK);
  ...
  const calc = useMemo(() => {
    ...
    const piWon = salaryWon * (piPercent / 100);
    const rsuValueWon = rsuShares * stockPrice;
    const encouragementWon = includeEncouragement ? ENCOURAGEMENT_BONUS : 0;
    const totalGross = piWon + rsuValueWon + encouragementWon;
  ...
  }, [salaryManwon, piPercent, rsuShares, stockPrice, includeEncouragement, creditRate, applyInsurance]);
    ...
    <div className="space-y-6">
      <section className="rounded-2xl border border-canvas-deep bg-white p-6 sm:p-8">
        ...
        <div className="mt-3 grid grid-cols-5 gap-2">
          {[5000, 7000, 9000, 12000, 15000].map((m) => (
            <button
              key={m}
              type="button"
        ...
        <p className="mt-2 text-xs text-faint">
          카카오 신입 5,000만원~ / 평균 8,500만~9,500만원 / 리더 1억~1.3억
        </p>
      </section>
      ...
      <section className="rounded-2xl border border-canvas-deep bg-white p-6 sm:p-8">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <label className="block">
            ...
            />
            <div className="mt-2 grid grid-cols-4 gap-1">
              {[40_000, 50_000, 60_000, 80_000].map((p) => (
                <button
                  key={p}
                  type="button"
        ...
        <p className="mt-3 text-xs text-faint">
          ...
          {" "}(2026 평균 1인당 약 670만원, 직급·평가에 따라 편차)
        </p>
      </section>

      <section className="rounded-2xl border border-canvas-deep bg-white p-6 sm:p-8">
        <h2 className="text-xl font-black mb-4">4단계 · 2026 격려금</h2>
        <label className="flex items-center gap-3 p-4 rounded-xl border border-canvas-deep cursor-pointer hover:bg-canvas/30">
          <input
            ...
            onChange={(e) => setIncludeEncouragement(e.target.checked)}
            className="w-5 h-5 accent-primary"
          />
          <div>
            <p className="font-bold">2026년 노사 합의 격려금 100만원 포함</p>
            <p className="text-xs text-faint mt-0.5">
              사측 제안. 연봉 인상률 6.8%와 별도. 최종 합의 시 지급.
            </p>
          </div>
        </label>
      </section>
      ...
      <section className="rounded-2xl border-2 border-primary bg-gradient-to-br from-primary/5 to-primary/10 p-6 sm:p-8">
        ...
        <div className="grid sm:grid-cols-3 gap-3 mb-5">
          ...
          <ResultCard label="RSU 가치" value={fmtManwon(calc.rsuValueWon)} sub={`${rsuShares}주 × ${(stockPrice / 10000).toFixed(0)}만`} />
          <ResultCard label="격려금" value={fmtManwon(calc.encouragementWon)} sub={includeEncouragement ? "2026 노사합의" : "포함 안 함"} />
        </div>

        <div className="rounded-xl bg-white border border-primary/30 p-5">
          <p className="text-xs font-bold text-faint mb-1">📊 총 보상 (세전)</p>
