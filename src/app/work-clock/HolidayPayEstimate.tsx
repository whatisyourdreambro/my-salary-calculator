...
export default function HolidayPayEstimate({ ordinaryHourlyDefault }: { ordinaryHourlyDefault: number | null }) {
  ...
  return <section className="rounded-2xl border border-border bg-card p-5 sm:p-6" aria-labelledby="holiday-estimate-heading">
    ...
    <div className="mt-5 grid gap-4 sm:grid-cols-3">
      <label className="min-w-0 text-sm font-medium">통상시급 (원)<NumberInput className={fieldClass} type="number" min="1" max="100000000" inputMode="decimal" value={hourly} placeholder={ordinaryHourlyDefault !== null ? Math.round(ordinaryHourlyDefault).toLocaleString("ko-KR") : "통상시급 직접 입력"} onChange={(event) => setHourly(event.target.value)} /></label>
    ...
    <label className="mt-4 flex items-start gap-2 text-sm leading-6"><input className="mt-1 h-4 w-4 shrink-0 accent-primary" type="checkbox" checked={eligible} onChange={(event) => setEligible(event.target.checked)} />상시 근로자 5인 이상 사업장이며, 해당 시간이 법정·약정 휴일근로이고 가산 규정 적용 대상입니다.</label>
    {result ? <dl className="mt-5 grid grid-cols-2 gap-4 rounded-xl border border-primary/25 bg-accent p-4 sm:grid-cols-4">{[["실제 근로임금", result.base], ["휴일 가산액", result.holidayPremium], ["야간 가산액", result.nightPremium], ["합계 · 세전", result.total]].map(([label, value]) => <div key={label}><dt className="text-xs text-muted-foreground">{label}</dt><dd className="mt-1 break-words text-lg font-bold tabular-nums text-link">{won(Number(value))}</dd></div>)}</dl> : <p className="mt-4 text-sm text-muted-foreground" role="status">{eligible ? "통상시급과 실제 근로·야간 시간을 확인해 주세요." : "적용 조건을 확인하면 예상 금액을 표시합니다."}</p>}
