...
export default function WeeklyTableInteractive({
  ...
  const [weeklySalary, setWeeklySalary] = useState(1000000);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <div className="bg-card p-6 sm:p-8 rounded-2xl shadow-xl border border-border">
            <h2 className="text-2xl font-bold mb-6">주급별 실수령액 시뮬레이터 (2026년 예상)</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              <div>
                <div className="space-y-6">
                  <div>
                    <Label htmlFor="weekly-salary" className="text-lg">주급: {weeklySalary.toLocaleString()}원</Label>
                    <Slider
                      id="weekly-salary"
                      min={100000}
                      max={5000000}
                      step={50000}
                      value={[weeklySalary]}
                      onValueChange={(value) => setWeeklySalary(value[0])}
                    />
                  </div>
                  <div>
        ...
        <div className="space-y-8">
            <div className="bg-card p-6 rounded-2xl shadow-xl border border-border">
                ...
                <div className="mt-4 space-y-2 text-muted-foreground">
                    <div className="flex justify-between"><span>세전 월급 (환산)</span><span>{(weeklySalary * 52 / 12).toLocaleString()}원</span></div>
