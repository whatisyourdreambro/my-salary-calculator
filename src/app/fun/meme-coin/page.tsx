...
export default function MemeCoinPage() {
 ...
 return (
 <main className="w-full min-h-screen bg-electric text-primary font-mono overflow-hidden relative selection:bg-primary selection:text-black pt-28">
 {/* Matrix Background */}
 ...
 <h1 className="text-4xl md:text-6xl font-black tracking-tighter text-navy mb-2">
 ...
 <p className="text-4xl font-bold text-navy tracking-tight">{formatNumber(remainingCapital)} KRW</p>
 </div>
 <button
 ...
 {coins.map(coin => (
 <div key={coin.id} className="bg-white/50 border border-canvas p-6 rounded-2xl hover:border-primary/50 transition-colors group">
 ...
 <div>
 <h3 className="text-xl font-bold text-navy">{coin.name}</h3>
 <p className="text-xs text-faint-blue">{coin.fullName}</p>
 </div>
 </div>
 <p className="text-sm text-faint-blue min-h-[40px] mb-6">{coin.description}</p>

 <div className="bg-electric/50 p-4 rounded-xl mb-4 border border-canvas">
 <p className="text-xs text-faint-blue mb-1">내 보유량</p>
 <p className="text-xl font-bold text-navy">{formatNumber(investments[coin.id as keyof typeof investments])} 원</p>
 </div>

 <div className="grid grid-cols-2 gap-2">
 <button
 onClick={() => handleInvestmentChange(coin.id as keyof typeof investments, -100000)}
 className="py-2 bg-canvas-dark hover:bg-primary/30 text-primary rounded-lg font-bold transition-colors"
 >
 -10만
 </button>
 <button
 ...
 <h2 className="text-2xl font-bold text-navy mb-2">시장 분석 중...</h2>
 ...
 {/* Stats Grid */}
 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
 <div className="bg-white/50 p-6 rounded-2xl border border-canvas">
 <p className="text-faint-blue text-sm">최종 평가액</p>
 <p className={`text-3xl font-bold ${result.profit >= 0 ? 'text-primary' : 'text-primary'}`}>
 {formatNumber(result.finalValue)} 원
 </p>
 </div>
 <div className="bg-white/50 p-6 rounded-2xl border border-canvas">
 <p className="text-faint-blue text-sm">수익금</p>
 <p className={`text-3xl font-bold ${result.profit >= 0 ? 'text-primary' : 'text-primary'}`}>
 {result.profit > 0 ? '+' : ''}{formatNumber(result.profit)} 원
 </p>
 </div>
 <div className="bg-white/50 p-6 rounded-2xl border border-canvas">
 <p className="text-faint-blue text-sm">수익률</p>
 <p className={`text-3xl font-bold ${result.profit >= 0 ? 'text-primary' : 'text-primary'}`}>
 ...
 {/* Chart */}
 <div className="h-64 bg-white/30 rounded-2xl border border-canvas p-4">
 ...
 <button
 onClick={resetGame}
 className="px-8 py-4 bg-white text-black font-bold rounded-xl hover:bg-canvas-deeper transition-colors flex items-center gap-2"
 >
