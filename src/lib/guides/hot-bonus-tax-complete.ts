...
<p>의료비·교육비·기부금 세액공제는 공제 대상 금액에 법정 비율(의료비·교육비 15%)을 곱해 세금에서 빼므로, 적용 세율이 35%인 사람이 받든 15%인 사람이 받든 같은 금액이 줄어듭니다. "세율이 높은 쪽이 의료비를 받아야 환급이 크다"는 설명은 소득공제와 세액공제를 혼동한 것입니다. 의료비에서 누가 유리한지를 가르는 것은 총급여 3% 문턱뿐입니다.</p>
