...
const FunCard = ({
    ...
    <Link
        href={href}
        className={`group relative overflow-hidden rounded-[24px] border border-slate-100 dark:border-slate-800/80 bg-white dark:bg-[#1E232E]
          shadow-[0_2px_8px_rgba(0,0,0,0.04),0_8px_24px_rgba(0,0,0,0.06)] dark:shadow-[0_4px_24px_rgba(0,0,0,0.4)]
          hover:shadow-[0_8px_32px_rgba(0,0,0,0.12)] dark:hover:shadow-[0_8px_40px_rgba(0,0,0,0.5)]
          hover:-translate-y-1 transition-all duration-200 block h-full ${featured ? 'md:col-span-2 md:row-span-2' : ''}`}
    >
        {/* 호버 그라데이션 */}
        <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-br ${color.replace('text-', 'from-').split('-')[0]}-50/80 via-transparent to-transparent dark:from-${color.split('-')[1]}-900/20`} />

        <div className={`relative z-10 flex flex-col h-full ${featured ? 'p-9' : 'p-6'}`}>
            <div className="flex items-start justify-between mb-5">
                <div className={`
                  ${featured ? 'w-16 h-16' : 'w-12 h-12'}
                  rounded-[18px] bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700
                  flex items-center justify-center group-hover:scale-110 transition-all duration-300
                `}>
                    <Icon className={`${featured ? 'w-8 h-8' : 'w-6 h-6'} text-slate-500 dark:text-slate-400 group-hover:${color} transition-colors duration-300`} />
                </div>
                {badge && (
                    <span className="px-3 py-1 rounded-full text-xs font-black bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 border border-blue-100 dark:border-blue-800">
                        {badge}
                    </span>
                )}
            </div>

            <h3 className={`${featured ? 'text-2xl' : 'text-lg'} font-bold text-slate-900 dark:text-white mb-2 tracking-tight group-hover:text-blue-600 transition-colors duration-200`}>
                {title}
            </h3>
            <p className={`text-slate-500 dark:text-slate-400 leading-relaxed ${featured ? 'text-base' : 'text-sm'} font-medium`}>
                {description}
            </p>

            <div className="mt-auto pt-5 flex items-center text-sm font-bold text-slate-400 group-hover:text-blue-600 transition-colors duration-200">
...
export default function FunLabPage() {
    return (
        <main className="min-h-screen bg-slate-50 dark:bg-[#191F28] pt-28 pb-20 px-4">
