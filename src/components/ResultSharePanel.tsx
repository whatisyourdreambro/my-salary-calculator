...
/** fromSave: opened by a card's in-app save button (A16), not by the preview button. */
type Preview = { key: string; approved: boolean; preparing: boolean; blob?: Blob; objectUrl?: string; error?: boolean; fromSave?: boolean };
...
export default function ResultSharePanel({
  ...
  // A card's in-app save request (A16) opens the preview below the card, so it is scrolled into
  // view once its image has loaded. Scrolling moves the viewport only: no element or height is added.
  const previewBox = useRef<HTMLDivElement>(null);
  const previewImage = useRef<HTMLImageElement>(null);
  const revealOnLoad = useRef(false);
  const reveal = () => { (previewImage.current ?? previewBox.current)?.scrollIntoView?.({ block: "center", behavior: "smooth" }); };

  async function openPreview(fromSave = false) {
    const currentRequest = ++request.current;
    const isCurrent = () => currentRequest === request.current && latestKey.current === key;
    // The OG-13 funnel counts the preview button only. A card's in-app save tap (A16) opens this
    // same preview to save an image, not to share, so it sends neither open nor approve.
    if (!fromSave) trackSharePreview("open", contentType);
    revealOnLoad.current = fromSave;
    const opened = { key, approved: false, fromSave };
    setPreview({ ...opened, preparing: !!getShareImage });
    ...
    try {
      ...
      if (!blob || !/^image\/(?:png|jpeg|webp)$/.test(blob.type) || blob.size === 0 || blob.size > 10 * 1024 * 1024) {
        setPreview({ ...opened, preparing: false, error: true });
        return;
      }
      const objectUrl = URL.createObjectURL(blob);
      setPreview({ ...opened, preparing: false, blob, objectUrl });
    } catch {
      if (isCurrent()) setPreview({ ...opened, preparing: false, error: true });
    }
  }
  ...
  // effect runs only per request; an already open preview is scrolled to instead of doing nothing.
  const requestPreview = useRef(() => {});
  requestPreview.current = () => {
    if (!resultIsCurrent) return;
    if (active) reveal();
    else void openPreview(true);
  };
  useEffect(() => { if (openPreviewRequest > 0) requestPreview.current(); }, [openPreviewRequest]);
  // Without an image to wait for (an error, or no getShareImage) reveal as soon as it settles.
  useEffect(() => {
    if (!revealOnLoad.current || !active || active.preparing || active.objectUrl) return;
    revealOnLoad.current = false;
    reveal();
  });

  // In Naver/KakaoTalk in-app WebViews the image is saved by long-press, so the <a download> link,
  // which fails silently there (A16), is not offered even after approval. Only true once a preview
  // exists (after a tap), so this client-only UA check never reaches server HTML.
  const saveByLongPress = !!active?.objectUrl && isImageDownloadRestricted(typeof navigator === "undefined" ? "" : navigator.userAgent);
  ...
  return <section className={`rounded-2xl border border-slate-200 bg-white p-4 text-slate-900 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 ${className}`} data-result-share-panel>
    ...
    {active && <div className="mt-4 border-t border-slate-200 pt-4 dark:border-slate-700">
      ...
      <div ref={previewBox} className="mt-3 rounded-xl bg-slate-50 p-3 dark:bg-slate-800">
        ...
        {active.objectUrl && <img ref={previewImage} onLoad={() => { if (revealOnLoad.current) { revealOnLoad.current = false; reveal(); } }} src={active.objectUrl} alt={en ? "Result image preview" : "공유할 결과 이미지 미리보기"} className="mt-3 max-h-[28rem] w-full object-contain" />}
        {/* In-app WebViews ignore file downloads; long-press on the image still saves it. This box
            stays visible after approval, so the hint does too. */}
        {saveByLongPress && <p className="mt-2 text-sm font-semibold">{en ? "Press and hold the image to save it." : "이미지를 길게 눌러 저장하세요."}</p>}
      </div>
      {!active.approved && <button type="button" disabled={active.preparing || active.error} className="mt-3 min-h-11 rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50" onClick={() => { if (!active.fromSave) trackSharePreview("approve", contentType); setPreview((current) => current?.key === key ? { ...current, approved: true } : current); }}>{en ? "Use this content for sharing" : "이 내용으로 공유"}</button>}
      {active.approved && <div className="mt-4">
        ...
        {active.objectUrl && !saveByLongPress && <a href={active.objectUrl} download="moneysalary-result.png" className="mt-3 inline-flex min-h-11 items-center rounded-xl border border-blue-600 px-4 py-2 text-sm font-semibold">{en ? "Save result image" : "결과 이미지 저장"}</a>}
      </div>}
