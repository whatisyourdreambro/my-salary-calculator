...
const PAGE_DESC =
  "삼성전자 OPI(초과이익성과금)·TAI(목표달성장려금) 계산기. 노조 전망 영업이익 360~370조 반영, 사업부별 1인당·건강보험료·세후 실수령·RSU 매도까지 무료 시뮬레이션.";
...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    answer:
      "2026년 5월 27일 조합원 투표(찬성 73.7%)로 최종 타결됐습니다. 핵심: (1) 임금 기본인상률 4.1% + 성과인상률 평균 2.1%, (2) DS부문 특별경영성과급 신설 — 영업이익의 10.5% 재원(상한 없음), 부문 공통 40% + 사업부 60% 분배, 세후 전액 자사주 지급(3분의 1 즉시, 나머지는 1·2년 잠금), 향후 10년 적용, 2027년부터 적자 사업부 최소 보장, (3) 기존 OPI 유지 — 단 회의록 기준 DS부문 OPI 재원은 2026년부터 EVA의 20%에서 영업이익의 10%로 변경되어, 특별경영성과급 재원(10.5%)과 합치면 영업이익의 약 20.5%가 DS부문 성과급 재원이 됩니다. 본 계산기의 10.5%·4:6 모델은 이 타결 내용을 반영한 것입니다.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "사업부 가중치는 사업부의 영업이익 기여도·전략적 중요도를 반영한 상대값입니다. 1.0이 표준이고, 0.55면 55% 가중, 0.06이면 거의 사업부 분배 제외(부문 균등 분배 위주 수령)됩니다. 본 계산기 디폴트는 메모리 1.0 / 공통 0.55 / 파운드리·시스템LSI 0.06으로, 초기업노조(SELU) 2026년 8월 공지 — 연봉 8,000만원 기준 OPI 포함 메모리 약 7.5억 / 공통 5.2억 / LSI·파운드리 2.7억 — 에 매칭되도록 역산 보정한 값입니다(영업이익 365조 기준 오차 1% 이내). 회의록 원본 가중치는 1.0/0.7/0.0이지만 노조 공지값과 정합되지 않아 공지값 매칭을 우선했습니다. 사용자가 회의록 원본 값으로 자유롭게 조정 가능합니다.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "성과급은 별도 분리과세가 아니라 연간 근로소득에 합산되어 누진세율(6~45%)이 적용됩니다. 여기에 지방소득세(소득세의 10%), 그리고 4대보험은 보수에 합산되어 정산되지만 국민연금은 기준소득월액 상한(2026년 7월부터 연 환산 7,908만원) 적용, 건강·고용보험은 상한 없음으로 처리됩니다. 특히 건강보험료(장기요양 포함)는 상한 없이 성과급 전액에 부과되어 체감 공제가 큰 항목이라, 본 계산기 결과 카드는 건강보험료를 별도 항목으로 표시하고 최종 세후 실수령액을 함께 보여줍니다. 일반적으로 성과급 실효세율은 20~38% 수준이며, 세액공제 활용도와 적용되는 4대보험 부과 방식에 따라 차이가 큽니다.",
  },
  {
    question: "주식 매도 제한이 매년 다른 이유는?",
    answer:
      "삼성전자가 임직원에게 지급하는 주식은 받은 단위마다 별도 매각제한 일정이 있습니다. 2026년 노사 합의의 DS부문 특별경영성과급은 지급분의 3분의 1은 즉시 매각 가능, 나머지는 3분의 1씩 1년·2년 매각이 제한됩니다(성과연도분은 이듬해 1월 지급). 일반 RSU는 받은 해 0%, 1~4년차 25%씩 누적처럼 회사·계약별로 다른 형태도 있습니다. 본 계산기는 매년 풀리는 비율을 사용자가 직접 입력할 수 있어 다양한 vesting 시나리오를 검토할 수 있습니다.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "사업부·연도별 영업이익에 따라 편차가 매우 큽니다. 초기업노조 2026년 8월 공지 기준 올해 예상 DS부문 영업이익은 360~370조이며, 연봉 8,000만원 기준 OPI 포함 메모리 약 7.5억 / 공통 5.2억 / LSI·파운드리 2.7억 규모로 안내됐습니다. 본 계산기 디폴트(영업이익 365조 가정, 평균 연봉 8,000만원)는 이 공지값에 맞게 보정되어 있습니다. '평균'이라는 단일 숫자보다, 위 계산기에 본인 사업부·연봉·그 해 영업이익을 넣어 본인 케이스를 직접 확인하는 것이 정확합니다. 본 결과는 공개 보도 기반 추정치입니다.",
  },
];
...
const HOW_TO_STEPS = [
  {
    name: "영업이익 입력",
    text:
      "DS부문 연간 영업이익(조원)을 입력합니다. 본 계산기의 부문·사업부 분배는 DS부문 기준입니다. 분기 영업이익이면 4배 환산. 노조 전망 360·365·370조와 30·100·1000조 등 다양한 시나리오를 빠른선택 칩으로 비교 가능합니다.",
  },
  {
    name: "사업부 인원·가중치 확인",
    text:
  ...
  },
  {
    ...
    text:
      "본인 연봉과 소속 사업부를 선택하면 본인 케이스의 세전·세후 성과급이 산출됩니다. 건강보험료(장기요양 포함)는 별도 항목으로 표시되고, 세액공제율·4대보험 부과 적용도 직접 조정 가능.",
  },
  {
    name: "다년도 RSU 매도 시뮬",
    text:
...
export const metadata: Metadata = {
  ...
  other: {
    "article:modified_time": "2026-08-31",
  },
};
...
const DIVISION_SCENARIOS = [
  {
    name: "메모리",
    profitRange: "2026 전망: DS 360~370조의 주력",
    perPersonRange: "OPI 포함 약 7.5억 (노조 8월 공지)",
    color: "#0145F2",
    note:
      "HBM·D램·NAND. 호황 시 가중치 1.0 기준 사업부 풀 최대 수혜. 금액은 연봉 8,000만원 기준 전망치.",
  },
  {
    name: "공통 (스태프·연구소)",
    profitRange: "DS 평균에 준함",
    perPersonRange: "OPI 포함 약 5.2억 (노조 8월 공지)",
    color: "#F59E0B",
    note: "특정 사업부에 속하지 않는 인력. 노조 공지 5.2억 매칭 디폴트 0.55 (회의록 원본 0.7).",
  },
  {
    name: "파운드리·시스템LSI",
    profitRange: "적자 ~ 소폭 흑자",
    perPersonRange: "OPI 포함 약 2.7억 (노조 8월 공지)",
    color: "#EF4444",
    note: "2026년 적자 → 노조 공지 2.7억 매칭 디폴트 0.06 (회의록 원본 0.0). 2027년~ 성과 가변.",
  },
];
        ...
        data={[
          softwareApplicationLd({
            ...
            featureList: [
              ...
              "본인 연봉 기준 세전·세후 실수령액",
              "건강보험료(장기요양 포함) 별도 표시·최종 세후 실수령",
              "노조 8월 공지 전망치(영업이익 360~370조) 원클릭 적용",
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <header className="text-center mb-8">
            ...
            <p className="text-sm text-faint-blue mt-3 max-w-md mx-auto leading-relaxed">
              ...
              분배 기준 (2026년 5월 노사 합의 타결·초기업노조 8월 공지 반영).
              사업부 1인당 평균
            ...
            <div className="space-y-4 text-sm leading-relaxed">
              <div>
                ...
                <p className="text-muted-blue dark:text-canvas-300">
                  ...
                  회의록 기준 DS부문 OPI 재원은 2026년부터 EVA의 20%에서
                  영업이익의 10%로 변경됐습니다. 디폴트 영업이익(365조)과
                  사업부 가중치는 초기업노조(SELU) 2026년 8월 텔레그램 공지 —
                  올해 DS부문 영업이익 360~370조 전망, 연봉 8,000만원 기준 OPI
                  포함 메모리 약 7.5억·공통 5.2억·LSI·파운드리 2.7억 — 에 맞게
                  보정했습니다. 사업부 디폴트 인원은 사업보고서 공시 자료를
                  참고했습니다.
                </p>
              </div>
              <div>
                ...
                <ul className="text-muted-blue dark:text-canvas-300 space-y-1 list-disc pl-5">
                  ...
                  <li>
                    <strong>노조 공지값 매칭 보정 적용</strong> —
                    초기업노조(SELU) 2026년 8월 공지(연봉 8,000만원 기준 OPI
                    포함 메모리 약 7.5억 / 공통 5.2억 / LSI·파운드리 2.7억)에
                    가장 근접하도록 가중치를{" "}
                    <strong>1.0 / 0.55 / 0.06</strong>으로 역산 보정했습니다
                    (영업이익 365조 입력 시 OPI 포함 약 7.54/5.22/2.69억 —
                    오차 1% 이내). 회의록 원본 가중치 1.0/0.7/0.0은
                    공지값과 정합 불가(공통 과대·파운드리 사업부 분배
                    0)하므로 공지값 매칭을 우선했습니다.
                  </li>
                  <li>
                    <strong>2026년 한정</strong> — 회의록상 적자
                    사업부(파운드리·LSI) 가중치 0이지만 노조 공지
                    전망(2.7억)이 부문 균등분+OPI를 웃돌아 디폴트를 0.06으로
                    뒀습니다. 2027년 이후는 사업부 성과에 따라 조정 (흑자 전환
                    시 가중치 ↑). UI에서 직접 입력하세요.
                  </li>
                  <li>
          ...
          <section className="mb-10" aria-labelledby="model-title">
            ...
            <div className="rounded-2xl bg-white dark:bg-canvas-900 border border-canvas-200 dark:border-canvas-800 p-6 space-y-4">
              <Step
                ...
                desc="상한 50%. 회의록 기준 2026년부터 DS부문 재원은 EVA 20% → 영업이익 10%로 변경. 사업부별 실지급률 상이(2025년분: MX 50%·DS 47%·VD 12% 등) — 계산기에서 조정 가능. 임계값 미달 연도에도 지급."
                highlight
              />
              <Step
                num="1"
                title="OPI2 재원 산정 (영업이익 × 10.5%)"
                desc="예) 365조 × 10.5% = 38.33조원. 영업이익 임계값(26~28년 200조, 29~35년 100조) 미달 시 OPI2만 0."
              />
              <Step
                num="2"
          ...
          <article className="prose prose-sm sm:prose-base dark:prose-invert max-w-none mb-10">
            ...
            <p className="text-muted-blue dark:text-canvas-300 leading-relaxed">
              ...
              비교하는 도구로 활용하세요. 초기업노조는 2026년 8월 공지에서
              올해 DS부문 영업이익을 360~370조로 전망했고, 본 계산기 디폴트는
              그 중간값인 365조입니다.
            </p>
            ...
            <ul className="space-y-1 text-muted-blue dark:text-canvas-300 leading-relaxed">
              ...
              <li>
                ...
                연 환산 7,908만원) 적용. 상한 없는 건강보험료(장기요양 포함)는
                결과 카드에 별도 항목으로 표시
              </li>
              <li>
