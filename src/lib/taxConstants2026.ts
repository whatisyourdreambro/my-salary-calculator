// src/lib/taxConstants2026.ts
//
// 2026년 세법·요율 단일 진실 소스 (Single Source of Truth).
//
// 기존엔 TaxLogic.ts 와 bonusTaxCalc.ts 두 파일에 같은 세율 상수가 중복
// 정의되어 있어, 2027년 세율 변경 시 두 곳 모두 손봐야 하는 위험이 있었음.
// 이 파일에 모아두고 두 파일이 import 해 사용.
//
// ★ 2026 블록을 제자리 수정 금지 — 연도 전환은 rates 인자/포인터 상수로 한다.
//   (TaxLogic.calculateSalary2026·bonusTaxCalc.calcBonusNet 의 선택 인자 rates,
//   calculator.ts calculateNetSalaryWithRates 의 NetSalaryRates.)
//   2026 귀속 연말정산(YearEndTaxCalculator·widget/year-end-tax)과 /table/2026 은
//   2027년에도 2026 요율을 써야 하므로 이 블록 값은 2026 으로 고정한다.

// ─────────────────────────────────────────────────────────────
// 종합소득세 누진세율 (소득세법 §55) — 2026년 기준 8단계
// ─────────────────────────────────────────────────────────────
export const TAX_BRACKETS_2026 = [
  { limit: 14_000_000, rate: 0.06, deduction: 0 },
  { limit: 50_000_000, rate: 0.15, deduction: 1_260_000 },
  { limit: 88_000_000, rate: 0.24, deduction: 5_760_000 },
  { limit: 150_000_000, rate: 0.35, deduction: 15_440_000 },
  { limit: 300_000_000, rate: 0.38, deduction: 19_940_000 },
  { limit: 500_000_000, rate: 0.40, deduction: 25_940_000 },
  { limit: 1_000_000_000, rate: 0.42, deduction: 35_940_000 },
  { limit: Infinity, rate: 0.45, deduction: 65_940_000 },
] as const;

// ─────────────────────────────────────────────────────────────
// 4대보험 요율 (근로자 부담분, 2026년 — 보건복지부 2025-08-28 고시 반영)
// ─────────────────────────────────────────────────────────────
export const INSURANCE_RATES_2026 = {
  /** 국민연금 — 근로자 4.75% (회사 4.75% 별도). 2026 연금개혁: 9%→9.5% (2026-01-01 시행, 2033년까지 매년 0.5%p씩 13%로). */
  NATIONAL_PENSION: 0.0475,
  /**
   * 건강보험 — 근로자 3.595%.
   * 2026년 전체 7.19% (회사 + 근로자 합), 근로자 절반 부담.
   * 2025년 7.09% → 2026년 7.19% (+0.1%p, 3년만의 인상).
   * 출처: 보건복지부 제15차 건강보험정책심의위원회 (2025-08-28).
   */
  HEALTH_INSURANCE: 0.03595,
  /**
   * 장기요양보험 — 건강보험료의 13.14%.
   * 2026년 소득 대비 0.9448% (2025년 0.9182%에서 인상).
   * 건보료 대비 비율: 2025년 12.95% → 2026년 13.14% (+0.19%p).
   * 출처: 보건복지부 (2025-09).
   */
  LONG_TERM_CARE_RATIO: 0.1314,
  /** 고용보험 — 근로자 0.9%. 변동 없음. */
  EMPLOYMENT_INSURANCE: 0.009,
  /** 지방소득세 — 소득세의 10%. 변동 없음. */
  LOCAL_INCOME_TAX_RATIO: 0.1,
} as const;

/**
 * 4대보험·지방세 요율 묶음의 형태 — INSURANCE_RATES_2026 과 같은 키, 값은 number.
 * (`typeof INSURANCE_RATES_2026` 은 as const 리터럴 타입이라 다른 연도 값을 받을 수 없다.)
 * TaxLogic.calculateSalary2026·bonusTaxCalc.calcBonusNet 의 선택 인자 rates 타입.
 */
export type InsuranceRates = {
  readonly [K in keyof typeof INSURANCE_RATES_2026]: number;
};

// ─────────────────────────────────────────────────────────────
// 국민연금 기준소득월액 상한·하한
// 매년 7월 1일 변경. 2026년 7월 ~ 2027년 6월 적용 기준 (현재 최신).
// 상한 637만→659만, 하한 40만→41만 (2026-07-01 인상). 출처: 국민연금공단.
// ─────────────────────────────────────────────────────────────
export const PENSION_BASE_2026 = {
  /** 월 상한 (원) */
  MAX_MONTHLY: 6_590_000,
  /** 월 하한 (원) */
  MIN_MONTHLY: 410_000,
  /** 연 상한 (원) — 성과급 합산 계산용 */
  MAX_ANNUAL: 6_590_000 * 12, // 79_080_000
} as const;

// ─────────────────────────────────────────────────────────────
// 월세 세액공제 (조세특례제한법 §95의2) — 2026년 귀속 현행법
// 정본 상수 — rent-tax-credit-2026 Client·연말정산 엔진이 공유 (2026-08-23 정합화)
// ─────────────────────────────────────────────────────────────
export const RENT_CREDIT_2026 = {
  /** 공제 대상 월세 한도 (연, 원) */
  CAP: 10_000_000,
  /** 총급여 상한 — 초과 시 공제 대상 아님 (종합소득 7,000만 초과자도 제외) */
  SALARY_CAP: 80_000_000,
  /** 17% 구간 상한 (경계 포함 → 17%) */
  SALARY_17_MAX: 55_000_000,
  RATE_HIGH: 0.17,
  RATE_LOW: 0.15,
} as const;

// ─────────────────────────────────────────────────────────────
// 연금계좌세액공제 (소득세법 §59의3, 2026-01-01 시행 현행 조문 확인 2026-09-25)
// 연금저축 600만원 이내 + 퇴직연금계좌(IRP 등) 합산 900만원. 공제율 12%, 총급여 5,500만원
// 이하(종합소득금액 4,500만원 이하)는 15%. ISA 만기 전환액은 10%(최대 300만원)만큼 한도 가산(§59의3④).
// 정본 상수 — 연말정산 엔진·/year-end-tax 공제 한도표가 공유 (2026-09-25)
// ─────────────────────────────────────────────────────────────
export const PENSION_ACCOUNT_CREDIT_2026 = {
  /** 연금저축계좌 공제대상 납입 한도 (연, 원) */
  SAVINGS_CAP: 6_000_000,
  /** 연금저축(600만 이내)+퇴직연금계좌 합산 공제대상 한도 (연, 원) */
  TOTAL_CAP: 9_000_000,
  /** 15% 구간 총급여 상한 (경계 포함 → 15%) */
  SALARY_15_MAX: 55_000_000,
  RATE_HIGH: 0.15,
  RATE_LOW: 0.12,
  /** ISA 만기 전환액 가산 비율·상한 */
  ISA_TRANSFER_RATIO: 0.1,
  ISA_TRANSFER_CAP: 3_000_000,
} as const;

// ─────────────────────────────────────────────────────────────
// 의료비·교육비 세액공제 (소득세법 §59의4②③, 2026-01-01 시행 현행 조문 확인 2026-09-25)
// 연말정산 엔진은 공제율·3% 문턱과 교육비 1명당 한도(educationChildSchool·educationUniversity 입력, 2026-09-26)를
// 쓰고, 의료비 한도는 /year-end-tax 공제 한도표가 표시에 쓴다.
// 산후조리원 200만원(출산 1회)은 시행령 — 2024-01-01 지출분부터 총급여 요건 삭제(국세청 상담센터).
// ─────────────────────────────────────────────────────────────
export const MEDICAL_CREDIT_2026 = {
  RATE: 0.15,
  /** 미숙아·선천성이상아 의료비 */
  PREMATURE_RATE: 0.2,
  /** 난임시술비 */
  INFERTILITY_RATE: 0.3,
  /** 총급여 대비 문턱 — 초과분부터 공제 */
  THRESHOLD_RATIO: 0.03,
  /** 본인·6세 이하·65세 이상·장애인 등 외 부양가족 의료비 한도 (연, 원) */
  DEPENDENT_CAP: 7_000_000,
  /** 산후조리원 비용 한도 (출산 1회당, 원) */
  POSTPARTUM_CAP: 2_000_000,
} as const;

export const EDUCATION_CREDIT_2026 = {
  RATE: 0.15,
  /** 취학 전 아동·초중고생 1명당 (연, 원) — 2026년부터 초등 2학년 이하 예체능 학원비 포함 */
  SCHOOL_CAP: 3_000_000,
  /** 대학생 1명당 (연, 원) */
  UNIVERSITY_CAP: 9_000_000,
} as const;

// ─────────────────────────────────────────────────────────────
// 보장성보험료 세액공제 (소득세법 §59의4①, 2026-01-01 시행 법률 제21221호 — law.go.kr 현행 조문 확인 2026-09-26)
// 만기환급금이 납입보험료를 넘지 않는 보험. 일반 보장성보험료(기본공제대상자가 피보험자) 12%,
// 장애인전용보장성보험료 15% — 보험료별로 각각 연 100만원을 넘는 금액은 없는 것으로 본다.
// 연말정산 엔진의 lifeInsurance 입력은 일반 보장성보험료(12%)다.
// ─────────────────────────────────────────────────────────────
export const INSURANCE_CREDIT_2026 = {
  /** 공제대상 보험료 한도 (연, 원) — 일반·장애인전용 각각 */
  LIMIT: 1_000_000,
  /** 일반 보장성보험료 공제율 */
  RATE: 0.12,
  /** 장애인전용보장성보험료 공제율 */
  DISABLED_RATE: 0.15,
} as const;

// ─────────────────────────────────────────────────────────────
// 출산·입양 세액공제 (소득세법 §59의2③, 2026-01-01 시행 법률 제21221호 — law.go.kr 현행 조문 확인 2026-09-26)
// 해당 과세기간에 출산하거나 입양 신고한 공제대상자녀가 첫째 30만원·둘째 50만원·셋째 이상 70만원.
// 8세 이상 자녀세액공제(§59의2①, CHILD_TAX_CREDIT_2026)와 별도로 더한다.
// ─────────────────────────────────────────────────────────────
export const BIRTH_CREDIT_2026 = {
  FIRST: 300_000,
  SECOND: 500_000,
  THIRD_PLUS: 700_000,
} as const;

// ─────────────────────────────────────────────────────────────
// 혼인에 대한 세액공제 (조세특례제한법 §92① [본조신설 2024. 12. 31.] — law.go.kr 현행 조문 확인 2026-09-26)
// 2026년 12월 31일 이전에 혼인신고를 한 거주자는 1회에 한정해 혼인신고를 한 날이 속하는 과세기간의
// 종합소득산출세액에서 50만원을 공제한다. 2024년 혼인신고분부터 적용, 부부가 각각 50만원(생애 1회)
// — 기획재정부 '2025년부터 이렇게 달라집니다'(2025-01-08). 2026년 귀속에서는 2026년 혼인신고만 해당한다.
// ─────────────────────────────────────────────────────────────
export const MARRIAGE_CREDIT_2026 = {
  /** 1인당 공제액 (생애 1회, 원) */
  AMOUNT: 500_000,
  /** 적용 혼인신고 기간 (KST, 경계 포함) */
  REGISTERED_FROM: "2024-01-01",
  REGISTERED_UNTIL: "2026-12-31",
} as const;

// ─────────────────────────────────────────────────────────────
// 근로소득공제 누진율 (소득세법 §47) — 2024년 표준 (2026 유지)
// ─────────────────────────────────────────────────────────────
export function earnedIncomeDeduction2026(annualSalary: number): number {
  if (annualSalary <= 5_000_000) return annualSalary * 0.7;
  if (annualSalary <= 15_000_000)
    return 3_500_000 + (annualSalary - 5_000_000) * 0.4;
  if (annualSalary <= 45_000_000)
    return 7_500_000 + (annualSalary - 15_000_000) * 0.15;
  if (annualSalary <= 100_000_000)
    return 12_000_000 + (annualSalary - 45_000_000) * 0.05;
  // 100,000,000 초과 — 14,750,000 + 초과분 2% (단, 한도 20,000,000)
  return Math.min(14_750_000 + (annualSalary - 100_000_000) * 0.02, 20_000_000);
}

// ─────────────────────────────────────────────────────────────
// 종합소득세 산출세액 (누진세율표 적용)
// ─────────────────────────────────────────────────────────────
export function calcIncomeTax2026(taxable: number): number {
  if (taxable <= 0) return 0;
  for (const b of TAX_BRACKETS_2026) {
    if (taxable <= b.limit)
      return Math.max(0, Math.round(taxable * b.rate - b.deduction));
  }
  return 0;
}

// ─────────────────────────────────────────────────────────────
// 자녀세액공제 (소득세법 §59의2) — 2025-01-01 시행 개정 현행
// 첫째 25만·둘째 30만·셋째 이상 1명당 40만 (8세 이상 기본공제 대상 자녀 기준).
// 1명 25만 / 2명 55만 / 3명 95만. 출산·입양(첫째 30만·둘째 50만·셋째+ 70만)은 별도.
// ─────────────────────────────────────────────────────────────
export const CHILD_TAX_CREDIT_2026 = {
  FIRST: 250_000,
  SECOND: 300_000,
  THIRD_PLUS: 400_000,
} as const;

export function childTaxCredit2026(children: number): number {
  if (children <= 0) return 0;
  if (children === 1) return CHILD_TAX_CREDIT_2026.FIRST;
  return (
    CHILD_TAX_CREDIT_2026.FIRST +
    CHILD_TAX_CREDIT_2026.SECOND +
    (children - 2) * CHILD_TAX_CREDIT_2026.THIRD_PLUS
  );
}

// ─────────────────────────────────────────────────────────────
// 근로소득세액공제 (소득세법 §59) — 산출세액 130만 이하 55%, 초과분 30%.
//
// 한도(§59②)는 계단식 상수가 아니라 구간 안에서 총급여에 비례해 체감하는
// 산식이다. 2026-09-06 전수검사 정정 전 구현은 각 구간의 '상단 값'(74만·66만·
// 50만)을 상수로 고정하고 3,300만 이하에는 한도를 두지 않아, 법정 한도보다
// 최대 연 30만원까지 관대했다(총급여 3,300만 107,000원 / 5,000만 80,000원 /
// 8,000만 160,000원 / 1.21억 이상 300,000원 과다공제).
//
//   1. 총급여 3,300만 이하                 : 74만원
//   2. 3,300만 초과 ~ 7,000만 이하        : 74만원 − (총급여 − 3,300만) × 8/1,000
//                                            (66만원 미만이면 66만원)
//   3. 7,000만 초과 ~ 1억 2,000만 이하    : 66만원 − (총급여 − 7,000만) × 1/2
//                                            (50만원 미만이면 50만원)
//   4. 1억 2,000만 초과                    : 50만원 − (총급여 − 1억 2,000만) × 1/2
//                                            (20만원 미만이면 20만원)
//
// ★ grossSalary 인자는 '총급여액'(연봉 − 비과세)이다. 비과세 포함 연봉을 넘기면
//   비과세 폭만큼 한도 구간이 밀린다(2026-09-06 이전 TaxLogic 이 그랬다).
// ─────────────────────────────────────────────────────────────
/** 소득세법 §59② 근로소득세액공제 한도 — 총급여액 기준 구간별 체감 산식 */
export function earnedIncomeTaxCreditLimit2026(grossSalary: number): number {
  if (grossSalary <= 33_000_000) return 740_000;
  if (grossSalary <= 70_000_000)
    return Math.max(660_000, 740_000 - (grossSalary - 33_000_000) * 0.008);
  if (grossSalary <= 120_000_000)
    return Math.max(500_000, 660_000 - (grossSalary - 70_000_000) * 0.5);
  return Math.max(200_000, 500_000 - (grossSalary - 120_000_000) * 0.5);
}

export function earnedIncomeTaxCredit2026(
  calculatedTax: number,
  grossSalary: number
): number {
  const credit =
    calculatedTax <= 1_300_000
      ? calculatedTax * 0.55
      : 715_000 + (calculatedTax - 1_300_000) * 0.3;
  return Math.min(credit, earnedIncomeTaxCreditLimit2026(grossSalary));
}

// ─────────────────────────────────────────────────────────────
// 2025년 레거시 요율 — "전년 대비 변화액" 산출 전용 (표의 changeValue 기준선).
// 2026 계산에는 절대 사용하지 말 것. 2027 개정 시 이 블록은 2026 값으로 교체.
// ─────────────────────────────────────────────────────────────
export const INSURANCE_RATES_2025_LEGACY = {
  /** 국민연금 — 근로자 4.5% (연금개혁 전) */
  NATIONAL_PENSION: 0.045,
  /** 건강보험 — 근로자 3.545% (전체 7.09%) */
  HEALTH_INSURANCE: 0.03545,
  /** 장기요양보험 — 건강보험료의 12.95% */
  LONG_TERM_CARE_RATIO: 0.1295,
  /** 고용보험 — 근로자 0.9% */
  EMPLOYMENT_INSURANCE: 0.009,
  /** 지방소득세 — 소득세의 10% */
  LOCAL_INCOME_TAX_RATIO: 0.1,
} as const;

export const PENSION_BASE_2025_LEGACY = {
  /** 월 상한 (원) — 2025.7~2026.6 적용 637만원 */
  MAX_MONTHLY: 6_370_000,
  /** 월 하한 (원) */
  MIN_MONTHLY: 400_000,
} as const;
