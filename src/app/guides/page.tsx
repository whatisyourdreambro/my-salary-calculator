...
export default function GuidesPage() {
  ...
  return (
    <main className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
      ...
      <div className="flex flex-wrap justify-center gap-2 my-8 sm:my-12">
        {categories.map(category => (
          <button
            key={category.id}
            onClick={() => setSelectedCategory(category.name === '전체보기' ? 'all' : category.name)}
            className={`px-3 py-1.5 sm:px-4 sm:py-2 text-sm font-semibold rounded-full transition-colors ${
        ...
        {filteredGuides.map(guide => (
          ...
          >
            <div className="p-6 flex-grow">
              ...
              <h2 className="text-lg sm:text-xl font-bold text-foreground group-hover:text-primary transition-colors duration-300">
                {guide.title}
              </h2>
