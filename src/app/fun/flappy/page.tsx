...
export default function FlappyGamePage() {
 ...
 FLAPPY <span className="text-electric">SALARYMAN</span>
 </h1>
 <p className="text-faint-blue">세금과 물가를 피해 월급을 지켜라!</p>
 </div>

 <div className="relative bg-electric rounded-2xl overflow-hidden shadow-2xl border-4 border-canvas">
 <canvas
 ref={canvasRef}
 width={400}
 height={600}
 onClick={jump}
 className="w-full h-auto cursor-pointer bg-electric touch-none"
 />

 {/* Score Overlay */}
 {gameState === "playing" && (
 <div className="absolute top-8 left-1/2 -translate-x-1/2 text-6xl font-black text-navy/20 pointer-events-none select-none">
 {score}
 </div>
 )}
 ...
 {/* Start Screen */}
 {gameState === "start" && (
 <div className="absolute inset-0 bg-electric/60 flex flex-col items-center justify-center text-white p-8 text-center">
 <Trophy className="w-16 h-16 text-primary mb-4" />
 <h2 className="text-2xl font-bold mb-2">준비되셨나요?</h2>
 <p className="text-faint-blue mb-8">화면을 터치하거나 스페이스바를 눌러 점프하세요.</p>
 <button
 onClick={startGame}
 className="px-8 py-4 bg-primary hover:bg-primary text-white rounded-xl font-bold text-xl transition-colors flex items-center gap-2"
 >
 ...
 {/* Game Over Screen */}
 {gameState === "gameover" && (
 <div className="absolute inset-0 bg-electric/80 flex flex-col items-center justify-center text-white p-8 text-center backdrop-blur-sm">
 <h2 className="text-4xl font-black text-primary mb-2">GAME OVER</h2>
 <div className="bg-electric p-6 rounded-xl border border-canvas mb-6 w-full">
 <div className="flex justify-between items-center mb-2">
 <span className="text-faint-blue">이번 기록</span>
 <span className="text-2xl font-bold">{score}</span>
 </div>
 <div className="flex justify-between items-center">
 <span className="text-faint-blue">최고 기록</span>
 ...
 <button
 onClick={startGame}
 className="flex-1 py-3 bg-primary hover:bg-primary rounded-xl font-bold transition-colors flex items-center justify-center gap-2"
 >
