...
- ★**2026-09-25 정정(감사 CON-01, 이 절의 전제 붕괴)**: 앵커는 꺼져 있지 않았다. AdSense 광고 형식 보고서상 앵커·사이드레일·모바일 전면은 **9/1 이전부터 게재 중**(9/1~9/9 앵커 25,431노출·$42.92·0.78/PV, 9/8 라이브 감사 '오버레이 형식 3/3 활성'). 9/21 ON 은 처치가 없으므로 **10/5 판정 취소 → 28일 형식 스냅샷**, 이 절의 효과 ×1.03~1.10 은 계상하지 않는다. '사이드레일 OFF'는 실행하지 않는다(운영자 9/25 승인 12번 현행 유지). 새 오버레이 변경은 별도 승인·10/10 조회 이후.
...
| 9/1~9/9 (2026-09-25 추가) | $14.84 | 3,628 | $4.09 | 5.93 | 52.8% | 499 |
| 9/10~9/23 (2026-09-25 추가 — 자동 인페이지 붕괴 창) | $9.51 | 3,200 | $2.97 | 4.13 | 66.6% | 465 |
| **8/27~9/23 28일 (2026-09-25 기준선)** | **$11.62** | **3,345** | **$3.47** | **4.77** | **59.2%** | **444** |

★**2026-09-25 기준선 메모(감사 CON-05, 위 행은 덮지 않는다)**: 28일 합 $325.23 · PV 93,664 · 노출 447,069 · 클릭 12,419 · 광고 요청 594,416 · 게재 92.37% · AV 59.17% · 자동 $263.48(81.0%) / 수동 $61.76(19.0%). 일 수익은 $11.62 로 종전 30일 $346(일 $11.53)과 같은 런레이트이고, 바뀐 것은 페이지 RPM·노출/PV 다(9/10 붕괴). 요청→노출 손실의 대부분은 모바일 전면(요청 83,384 vs 노출 29,059 — 페이지마다 요청하고 이동할 때만 보여 주는 설계, 커버리지 94.78% 로 채움 문제 아님)과 멀티플렉스(커버리지 34.25%). AV 가 9/10 에 53~55% → 65~69% 로 뛴 것은 자동 인페이지가 빠진 구성 효과다.
**정의(9/25 신설)**: **자동 인페이지 노출 = '광고 형식' 인페이지 행 − 같은 날 수동 광고 단위 노출 합계**(인페이지 행에 수동 유닛이 포함됨 — 형식 행 합이 사이트 합과 맞으려면 수동 $61.76 이 인페이지 안에 있어야 함). 자동 인페이지/PV: 9/1~9/9 2.99 → 9/10~9/23 0.72 → 9/24 23:25 `4d80ce3e` 복구, P0 조회 10/2·10/9(`docs/ad-experiments.md` 2026-09-25 절 2). 출처: AdSense 콘솔 읽기 전용 추출(2026-09-24 KST, USD).
...
2. **수동 유닛 클릭의 단가가 낮다**(CPC $0.013~0.021 vs $0.044~0.059 — 누적 3.38배 · 8월 3.69배 · 창별 2.5~3.9배) — CTR 은 4~5%(7월·8월 초)로 디스플레이 통상치(0.5~1.5%)의 3~5배. "높은 CTR + 극저 CPC"는 우발 클릭(광고가 버튼·입력·결과 카드 등 상호작용 요소 옆에 있어 잘못 눌림)에 구글이 단가를 깎는 전형 패턴. 7/13~8/23 사이 수동 CTR ≥4% 인 날이 33일. 8/25 이후 2.2~2.9%·CPC $0.021 로 완화. ★**2026-09-06 특정 완료**(광고단위 CSV): 우발 클릭 서명(CTR≥3.5% AND CPC≤$0.015) 적중은 **결과창_본문(5584143639) 단독** — CTR 4.147% · CPC $0.01196. 다만 저CPC 대역의 단위는 "수동 풀 전체"가 아니라 **인콘텐츠 3유닛**(결과창 $0.01196 · 가이드중간 $0.01286 · 구 GUIDE_MID 앵커 $0.01300)이며 이들이 수동 클릭의 **87.68%**(7,861/8,966)를 만든다. 상단 $0.03193 · PC_날개 $0.30750 은 정상~고가이므로 "CPC 판별력 0"은 과장이다(AND의 구속 조건은 CTR). ★**단 4.147%는 약 9.5개월 누적치**이고 수동 합계 CTR은 7월 4.09% → 8월 3.57% → 9/1~9/4 2.20%이므로 **현행 발화 여부는 이 표로 판정 불가** — 최근 28일 창 지정 units CSV로만 재판정(콘솔팩 세션 1 ③). 리스크 규모: 결과창 = 수동 수익의 **64.48%** · 사이트 누적 수익의 **9.98%**($84.67/$848.00). 위치 이동·간격 확대는 **승인 M(권고: 보류)**, "결과 카드와 광고 사이 분리선/여백" 변형은 광고 위 UI 금지로 승인 상정 이전에 기각.
...
| PV/세션 | **1.16** (2026-09-06 정정 — AdSense PV 4,363 ÷ GA4 세션 약 3,751. 종전 "1.5~2"는 PV 역산 가정) | ×1.3~1.5 | 계산기 이용 후 이탈 구조. 100배 계획 §2와 같은 값 |
| 세션 | **일 약 3,751**(GA4 9/1~9/4 실측 = 일사용자 3,458 × 세션/사용자 1.0847. 종전 "2,200~2,900"은 PV 역산치) | 채널 제약 | ★2026-09-06 정정: 구글은 **일 26.1클릭(최근 7일 8/28~9/3) · 18.4(최근 30일) · 28.1(8/23~9/3)**이다. 종전 "구글 일 ~10클릭(3개월 비브랜드 33클릭)"은 그 측정창(5/15~8/14 = 7.17클릭/일)에서는 **정확했고 지금 낡았을 뿐**(정정폭 ×2.6~3.6). "비브랜드 33"은 3개월 queries.csv 귀속 표본(256) − 브랜드(223)이며 같은 창 속성 총계 660 기준 실제 비브랜드는 ≈437이다(관측 하한을 총량으로 읽지 말 것). 361일 기준 브랜드 315클릭 = 25.9%, 비브랜드 ≤899(74.1%). 네이버는 GA4 소스 실측 완료(165,511세션 = 67.7%, 100배 계획 F1). 네이버 외부사이트 점유 9.5%·AI 브리핑 2026년 말 40% 확대 = 구조적 하방 |
| CPA | 일 $0~1 | $5~15 | 활성 오퍼 2종(신용조회, 건당 수천 원). 카드·증권·대출비교 머천트가 LinkPrice에 있어야 상한 도달 |
| 구글 | **일 26.1클릭(최근 7일) / 세션 일 30~45** | 12개월 **일 29~147클릭(중립 61) ≈ 세션 49~250(중립 103)** | ★2026-09-06 재산정(GSC 361일 실측, 100배 계획 F14·§8). 종전 "일 10클릭 → 12개월 100~300클릭"은 기준선이 낡고 상한이 과대했다. 회사 연봉·연봉계산기 머리쿼리는 잡플래닛·사람인·인크루트 독점. 이길 수 있는 장르로 적었던 **성과급 계산기는 이미 1~2.5위로 소진**(F17) — 새 헤드룸은 /salary-db/[id] 466p(7,894노출·217클릭·10.68위) · 헤드 계산기 7쿼리(54노출·0클릭·66.96위) · 실수령액 클러스터 159쿼리(569노출·21.58위) · /guides 42p(1,274노출·20클릭). 수익 기여는 12개월 후에도 총수익의 1.3~6.7% |
...
| _routes.json exclude(프리렌더 가족 Worker 우회 ≈1,050 URL) | 대기(9/21 L08a 와 같은 날 이후) | crawl-performance-1: middleware 우회 부작용(non-www 301·봇 403 상실 → CF 규칙 대체 확인)·RSC 프리페치 MPA 폴백·GA4 page_view 영향·TTL=규칙 B 동일·Purge 런북. ★**2026-09-25 예외(감사 B2 `764e5f56`·`4e123d80`)**: 빌드가 만드는 크롤러 파일 4종(`/robots.txt`·`/sitemap.xml`·`/rss.xml`·`/rss-companies.xml`)과 `apple-touch-icon` 2종만 먼저 exclude(26+1 규칙). 이 게이트의 대기 사유는 HTML 전용(RSC 폴백·GA4 page_view·HTML 캐시)이라 XML/TXT/PNG 에는 해당하지 않고, 미들웨어 matcher 가 원래 이 경로들을 건너뛰어 www 301·봇 403 손실도 없다(테스트로 고정). Cache-Control 은 `public/_headers`. 프리렌더 HTML 가족 ≈1,050 URL 은 계속 L08a HIT 확인 대기 |
...
| L02' | ~~광고단위 CSV 2장(8/3~8/16·8/17~8/30)~~ → **최근 28일 1장 + 9/6~9/12 기간 지정 1장** | [콘솔 10분] | ★2026-09-06 취소·교체: 전 창에 display-2 유닛이 1.03일만 존재(f5c1375 8/15 23:21)하고 계정 누적표에는 날짜 열이 없어 2장을 받아도 판정 불가 — 실험 #1은 '판정 불가·현상 유지' 확정. 새 목적은 ① 최근 28일 = 우발 클릭 현행 발화 재판정 ② 9/6~9/12 = 27a692c(IN_ARTICLE) 회수 판정. 정본 절차는 콘솔팩 |
...
| L04' | 앵커 ON (9/21) + 인피드 유닛 발급 + 자동광고 설정 캡처 | [승인④·콘솔 10분] | ~~×1.03~1.10~~ **0**(2026-09-25 삭제 — 앵커 9/1 이전부터 ON, 처치 없음) | §1-2 조건. 사이드레일·동적앵커 OFF. 판정 10/5. ★9/25: 앵커 부분 취소(10/5 = 28일 형식 스냅샷), 오버레이 현행 유지(운영자 승인 12번). 남은 것은 인피드 발급(A07 과 묶음, 10/25 주)·설정 화면 캡처(저장 금지) |
...
| M01 | 성과급 계산기 장르 구글 전선 — ★**2026-09-06 원안 폐기·재범위화**(100배 계획 §1 F17·§5·§9). 원안 타깃 2개(sk-hynix-bonus "SK하이닉스 성과급 계산기" · samsung-bonus "OPI/TAI 계산" title/H1 정확 일치)는 **무효** — 전자는 queries.csv 830행 전수 수요 0건(상한 41노출, 이미 3클릭/4.93위), 후자는 "opi2 계산기" 1.31위·"삼성전자 opi2 계산기" 1.43위로 개선 여지 없음. 남는 것: (a) **홈 H1 헤드 계산기 정합 단독 선행**(연봉 계산기 57.54 · 급여 73.75 · 월급 71.60 · 실수령액 61.25위, 7쿼리 54노출/0클릭 — 2줄 유지·히어로 높이 불변·광고 위 높이 증가 0) (b) /calc 저CTR 4종 스니펫(우선순위 낮음). 뉴스 5편 앵커텍스트 일치·경쟁 기능 격차 보강은 유지 | [코드] | ★**효과 목표 삭제** — 종전 "구글 일 10→25~40클릭(×2.5~4)"은 배포 전인 8/23~9/3에 이미 28.08클릭/일로 밴드를 충족했다. 재범위화안의 기대는 배수 미계상, (b)는 월 5클릭 내외 | 9/20 GSC 28일 "성과급" 필터 재수출 후. 신규 페이지 0. 판정은 **커스텀 기간 2창(8/9~8/22 vs 8/23~9/5)** + **/calc/samsung-bonus 단독과 "나머지 사이트" 분리 집계** + KR 필터. L18의 12/15 description 교체는 이 판정(10/19~26) 본 뒤 |
...
| L10' | 회사 description 공시 평균연봉 주입 — **축소판**: sourceUrl이 dart.fss.or.kr/alio.go.kr AND fiscalYear 2025 AND 공시 평균이 title 신입~시니어 범위 내인 회사(≈140곳, 빌드 시 실측) | [승인⑧] | 위생(구글 +0~0.15클릭/일, 네이버 미측정) | title 절대 불변. 프리픽스 아닌 후미 삽입, 150자 내. **같은 커밋에서 대상 회사 lastUpdated 승격 → 그 후 rss-companies 제출**(M08). 시점 9/28 이후·R2 타깃과 비동시. 판정 GSC 28일 국가=KR 필터. ★**2026-09-25 구현 완료·배포 대기**: 대상 실측 **164곳**(DART 주입 152 + 수기 DART 7 + 알리오 5, 산정치 basis 제외). 문구 = 기존 자체 추정치 구절 + 꼬리 '…로그인 없이 확인하세요.'(A7', 전 430곳) + 후미 ' 공시 평균연봉 {카드 헤드라인}(2025 사업연도).'(대상만) · 'N월 업데이트 기준' 삭제(COMP-06) · 최대 150자 · og:description=meta. 수정일 승격은 데이터 lastUpdated 가 아니라 **페이지 수정일**(`companyPageModified` → sitemap lastmod·RSS pubDate)만 2026-10-01 로 — 바뀐 것은 메타 문구라 배지·Dataset dateModified 는 그대로. **빌드 시점 게이트**(`COMPANY_META_DISCLOSED_DATE`, 10/1 00:00 KST — 운영자 승인 적용일, 켜는 빌드는 10/1 09:00 KST 이후): 그 전 빌드는 description·공시 카드·FAQ·수정일이 병합 전 main(bd68d860)과 바이트 동일(테스트 고정본 대조)이라 먼저 합쳐져도 안전. A4' 출처 라벨 동봉 — 같은 게이트 뒤(주입 카드 '출처(공시 원문):' + 출처 문구 꼬리 ' — OpenDART 수집' 는 표시에서만 생략(데이터 문구 불변, FAQ 도 같은 규칙), 수기 '원문:'/'보도:' — 카드 높이 증가 0 실측). 배포 절차는 `docs/next-upgrade-plan-2026-09-11.md` §1 10/1 L10' 행 |
| L13b | opiData.ts 단일 소스(OPI 실지급률 page.tsx 3곳+Client 기본값 `FIXED_OPI1_RATE` 치환) + TAI_RATES_2026_H2 null 슬롯 + 7종 계산기(LG엔솔·네이버·카카오·포스코·LG화학·HD현대·삼성SDI) Client 인라인 데이터 모듈화 + SK 타결 5점 동기화 키트 초안 + 발표 런북(docs/growth-playbook §3: D0 보도→상수→배포→CF 퍼지→GSC URL 검사→D+1 확인→D+3 리포트 갱신) | [코드] | 동결기 갱신 가능화(배수 0) | `calcSamsungBonusNet` 791/553/252%·MX 50% 회귀 테스트 선커밋. jsdom 추가 금지. 1/15~31 OPI 배너·12/15~31 TAI H2 배너는 **데이터 non-null 게이트**(발표 전 추정 카피 금지). **① 분할 선행 완료 2026-09-05(B12)** — 7종 계산기(lg-energy·naver·kakao·posco·lg-chem·hd-hyundai·samsung-sdi) Client 인라인 상수 → data.ts 모듈화 + data.test.ts 동결값 14건 + verify-bonus-data.mjs Client+data.ts 합산 스캔 + SK PS 타결 동기화 키트 docs/drafts/sk-ps-sync-kit-2027.md(5점·D0~D+3 런북). **② ✅ 2026-09-21 완료(`22eb3bad`)** — opiData.ts(OPI_ACTUAL·OPI_LATEST·OPI1_DEFAULT_RATE 파생)·taiData.ts H2 null 슬롯+TAI_LATEST 라벨 파생·annualOp.ts(ANNUAL_OP_2026_PRELIM 1/8 트리거)·model.ts 순수 분리·data.test.ts 잔존 문자열 게이트(허용 목록). **선행분 ④ 완료(B11)**: src/data/opiAnnouncement.ts OPI_2026_ANNOUNCEMENT(발표 전 announced=false·rate/date/source null 강제, 테스트가 불변식 검사) + seasonalCalendar.ts 1월 20~31 OPI 게이트 항목(requires:"opiAnnounced", 카드공제 항목 앞). 게이트는 getCurrentSeasonal(now, { opiAnnounced }) 인자 주입 — ★홈 배너 연결(src/components/SeasonalBanner.tsx:18 을 getCurrentSeasonal(now, { opiAnnounced: OPI_2026_ANNOUNCEMENT.announced }) 로 한 줄 교체)은 소유 범위 밖이라 미적용, 발표 런북(growth-playbook §3)에 포함. 발표 후 절차: opiAnnouncement.ts 4필드 채움 → SeasonalBanner 한 줄 → vitest. **[운영자 한 줄 결정 = 100배 계획 승인 H]** 1월 홈 배너 1순위 = OPI(1/20~31, 카드공제보다 우선) — 코드는 이 순서로 배치·게이트 잠금 상태, 결정이 다르면 게이트를 열지 않고 항목만 남긴다(카드공제 자연 폴스루). 2/1~5 연장은 medical(2/1~15)과 겹쳐 별도 결정 |
| L07' | §12-2 ⑥ 데스크톱 SIDEBAR 300x600 **단독** 이식(성과급 23→장문 4→/home-loan, SidebarAd만·skyscraper 병기 금지) | [승인⑨] | 피크 ×1.01~1.03 | **10/17 배포 → 10/31 판정** 가능할 때만. 불가 시 2월. #2a·CLS 정비와 분리. 데스크톱 비중 실측(9/13 번들) 30% 미만이면 강등. ★**강등 후보(2026-09-12 S3-4 판정)**: 9/8 AdSense 기기 분해에서 데스크톱 수익 비중 16.6% + 자동광고 사이드레일 ON 과 지면 중복 — 9/13 GA4 기기 비중(운영자 내보내기)으로 확정. 확정 전 코드 선행 금지 |
| C6 | ✅ **2026-09-11 `f2379fa` 해소(삼성 1쪽)** — 개인 세후 결과 직하에 CalcResultAd 를 두고 ResultNextLinks·OfferSlot 은 그 아래로. 타 22쪽은 OfferSlot 미배치 상태(9/12 S1-2 BonusNextLinks 도 OfferSlot 없음) → securities 활성 전제 충족. (원문: `samsung-bonus/Client.tsx:545` ResultNextLinks 내 OfferSlot 이 InArticleAd·CalcResultAd **위**) | [승인⑩ → 운영자 9/11 '광고 위치 변경 허용'으로 실행] | 0 (12월 securities 오퍼 활성의 전제) | 이동 없이 offers.json 스위칭으로 활성화하면 결과 직하 광고 앞에 CPA 카드 = 8/16 급락 패턴. 동결기 수정 불가 → 10월 필수 |
...
| L16' | 10/19 GSC 커버리지 재수출 → /salary-db/listed 219 색인률 산출(≥60% 게이트) | [콘솔 15분] | 0 | lite 본문 보강은 **이미 배포됨**(추이·순위·동종·환산 전부 존재) → 삭제. 통과 시 Phase 2 발행은 **2027-02**. ★**2026-09-25 판정식 개정(감사 SEO-CRAWL-08·NV-7, 10/19 자료 도착 전 사전 등록)**: ① **크롤 커버리지 = 1 − 발견됨-미색인 ÷ 섹션 전체** ② **색인률 = 색인 ÷ (색인 + 크롤링됨-미색인)**. 커버리지 **70% 이상**이고 GSC 크롤 통계 호스트 상태 '서버 연결'이 **14일 연속 양호**일 때만 ②로 품질(≥60%)을 판정한다. 아니면 **'판정 불가(크롤 제한)'** — Phase 2 를 열지 않고(§8-2) 캐시 규칙 B/C 적용 28일 뒤 재측정. 보조 지표(네이버 76.6% 채널): 서치어드바이저 사이트 진단 색인·색인제외 추이(lite 배포 전후)와 네이버 웹문서에서 lite 표본 10개 제목 검색 노출 여부 — 네이버 2026-07-06 스팸 사례의 '동일·유사 템플릿 대량 생성'에 걸리는지 확인. 9/21 현황(참고): 발견됨-미색인 792 중 /salary-db/listed 계열 246행(업종 28 포함, 마지막 크롤 없음) — 지금 재면 lite 커버리지는 0% 에 가까워 '판정 불가'. 크롤 요청 목적 중 발견 3%, 서버 연결 실패율 9/13~9/22 매일 6.5~14.7% |
...
- 11/7 사이드바 판정(10/17 배포 시). 12/1·1/2 시즌 세트 상수 교체(1/2 교체 전 확인 2건 — ① 공무원 2027 봉급표 라벨(예산안 3.9% → 확정 여부) ② 국세청 간소화 오픈일 공지 시에만 날짜 카피 보강. 삼성 TAI(12월)·OPI(1월) 링크는 라벨만, 지급률 수치는 발표 후 별도 커밋). 12/15 연말정산 라벨 교체. 12/20 securities 오퍼(C6 완료 전제 — ✅ 9/11 충족). 12월 TAI H2·1월 셀트리온·1월 말 OPI·2월 SK PS 발표 반영 ≤24h(런북) + 배포 후 CF Purge.
...
| **9/7 세션 1** | ① GA4 관리→데이터 수집 및 수정→데이터 보관→14개월 ② 관리→맞춤 정의→맞춤 측정기준(이벤트 범위) slot_kind·position·calc_type·offer_id·vertical 5개 ③ AdSense 보고서→분류 '광고 단위'→**최근 28일 1장**(8/3~8/16·8/17~8/30 2장은 2026-09-06 취소 — 콘솔팩 참조) (+ 하위 2026-09-05: 맞춤 채널 그룹 네이버=Organic Search·page_path 집계·광고단위 점검쌍 7종·position 확인·눈 확인 3건 — 콘솔팩) | 20 |
...
11. ~~성과급 계산기 OfferSlot 광고 아래 이동(12월 오퍼 활성 전제)~~ → ✅ **2026-09-11 `f2379fa`(C6) 로 해소** — 삼성 1쪽의 OfferSlot 이 CalcResultAd 아래로 이동, 타 22쪽은 OfferSlot 미배치(9/12 S1-2 BonusNextLinks 도 OfferSlot 없음). 승인 불요 → 목록에서 제외(번호는 참조 안정성을 위해 유지)
...
★2026-09-25 전제 정정(행은 유지): 9/30·10/31 행의 '앵커'는 처치가 없어(9/1 이전부터 ON) 배수에서 빠진다 — 10/31 은 '⑦ 위치 이동(승인 M, 10/11 주)·인피드 중 판정 통과분'만. 그리고 9/10~9/24 자동 인페이지 붕괴로 9월 실측 일 수익이 이 표의 하한 근처($9.5/일)까지 내려갔으므로, 9/30·10/31 행은 **9/24 복구의 P0 결과(10/2·10/9)를 먼저 기준선으로 다시 잡은 뒤** 읽는다.
...
| CPA 시즌 OfferSlot 8곳·securities 활성(L15 C3/C4) | 조건부 | 머천트 실재 + C6 위치 수리(✅ 9/11 f2379fa) + 광고 아래 라인 검증 |
...
2. 실험 판정: 광고단위 축 우선, 창 내 클릭<50이면 28일 연장, 미달 시 '판정 불가·현상 유지'. 판정 불가는 확장 게이트를 해제하지 않는다(27a692c 병합 게이트만). ★**단서 추가(2026-09-06)**: **창을 구성할 수 없거나**(입력 CSV에 날짜 차원이 없거나 대상 유닛이 전 창에 존재하지 않는 경우) **연장 후에도 도달 불가가 산술적으로 확정되면 연장 없이 즉시 종결**한다 — 창이 만들어지지 않으면 28일 연장도 판정을 만들지 못한다(실험 #1 실사례).
   - ★**규칙 충돌(미해소) — 표본 대상 정본 부재**: 위 '창 내 클릭<50'의 대상이 **실험 유닛**(display-2, 19클릭)인지 **잠식 탐지 표본**(결과창+인아티클 클릭 합, `scripts/adsense-report.mjs:758` sampleB = 7,085)인지 정본이 이 문서에 없다. 두 해석은 실험 #2a~c 확장 게이트의 개폐를 반대로 만든다. → **승인 L**(100배 계획 §6 · docs/ad-experiments.md 승인 대기표)로 운영자 한 줄 확정. 확정 전까지 이 규칙은 **실험 유닛 기준으로 잠정 적용**하고, 스크립트의 판정 표본 변경은 승인 L 이후에만 한다.
