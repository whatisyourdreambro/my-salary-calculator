...
export default function InstallmentPage() {
 ...
 <CreditCard size={13} aria-hidden="true" /> 신용카드·카드론
 </span>
 ...
 className={`p-3 rounded-xl border text-xs font-bold transition-all ${months === p.months && annualRate === p.rate ? "bg-primary text-white border-primary" : "bg-white border-canvas text-muted-blue hover:border-primary"}`}>
 <p>{p.label}</p>
 ...
 <div className="text-center"><p className="text-navy/60 text-xs mb-1">총 납부액</p><p className="text-navy font-black">{fmt(r.totalPayment)}원</p></div>
 <div className="w-px bg-white/20" />
 <div className="text-center"><p className="text-navy/60 text-xs mb-1">총 이자</p><p className="text-navy font-black">{fmt(r.totalInterest)}원</p></div>
 <div className="w-px bg-white/20" />
