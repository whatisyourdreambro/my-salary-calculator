...
| 날짜 | 할 일 | 계획 ID | 조건·주의 |
|---|---|---|---|
| **9/28(월)부터 매주 월요일 5분** | 서치어드바이저 → 리포트 → 검색 유입 → 최근 7일 **상위 30 페이지·상위 30 검색어** 캡처 또는 내보내기 → `docs/metrics-log.md` 에 한 줄(날짜·상위 5 페이지 클릭·특이 변화). 변경마다 시계열을 남기는 용도 | NAVER-02 | 공변량 메모: 9/25 /job 억 표기(B14)·9/26 파비콘·10/1 L10'·10/14 봉급표. 네이버 자료는 약 1주 늦게 갱신된다 |
| 9/26 배포 뒤(파비콘) | 서치어드바이저 → 요청 → 웹 페이지 수집 → `/` 1건과 상위 몇 쪽 **수집 요청** | NAVER-01 | 파비콘 파일 URL 은 그대로다(바꾸지 않음). 7~14일 뒤 네이버 모바일·통합검색 결과의 파비콘이 파란 사각형이 아닌지 확인 |
