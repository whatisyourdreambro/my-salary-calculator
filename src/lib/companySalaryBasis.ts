...
export function buildCompanySalaryFaq(
  ...
  // 금액 표기: 1억 이상은 "1억 5,800만원"(formatManwonKorean), 1억 미만은 종전 "N,NNN만원" 과
  ...
  return [
    {
      ...
      answer: disclosed
        ? `${name}의 공시 기준 직원 평균연봉은 ${disclosed.fiscalYear} 사업연도 ${formatManwonKorean(disclosed.avgSalaryManwon)}입니다. 공시 인용 출처: ${disclosed.source}. 신입 초봉이나 개인 지급액과는 다르며, 성과급 포함 범위·집계 대상·산정 방식은 공시 카드의 출처와 주의사항을 확인하세요.`
    ...
    },
    {
      ...
      answer: `본 DB의 ${name} 신입 기본급 추정은 약 ${baseLabel}, 평균 인센티브를 더한 세전 총연봉 추정은 약 ${totalLabel}입니다. 직원 전체 공시 평균과 다른 기준이며, 직무·학력·입사 시점·성과급 지급 조건에 따라 실제 첫해 금액은 달라집니다. 위 직급별 연봉표의 신입 행은 이 추정 총연봉을 사용합니다.`,
    },
  ];
}
