...
const DeductionItem = ({ label, value, iconBg, iconColor }: DeductionItemProps) => (
 ...
 <span className="font-semibold text-muted-blue text-[15px]">{label}</span>
 </div>
...
export default function SalaryResultCard({
 ...
 <div className="bg-white rounded-[32px] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-canvas/50">
 <div className="flex justify-between items-center mb-2">
 <span className="text-sm font-bold text-faint-blue">예상 월 실수령액</span>
 </div>
 ...
 <h3 className="text-sm font-bold text-navy">공제 상세 내역</h3>
 <Info size={18} className="text-faint-blue cursor-pointer hover:text-muted-blue transition-colors" />
 </div>

 <div className="divide-y divide-canvas">
 <DeductionItem label="국민연금" value={breakdown.pension} iconBg="bg-canvas" iconColor="text-electric" />
 <DeductionItem label="건강보험" value={breakdown.health} iconBg="bg-primary/5" iconColor="text-primary" />
 <DeductionItem label="고용보험" value={breakdown.employment} iconBg="bg-primary/10" iconColor="text-electric" />
 <DeductionItem label="소득세" value={breakdown.incomeTax + breakdown.localTax} iconBg="bg-canvas" iconColor="text-primary" />
 </div>
 </div>
 ...
 <p className="text-[13px] text-faint-blue leading-relaxed font-medium">
