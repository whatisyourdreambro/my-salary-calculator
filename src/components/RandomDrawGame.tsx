...
export default function RandomDrawGame() {
 ...
 className="bg-electric/40 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl relative overflow-hidden"
 >
 {/* Background Glow */}
 <div className="absolute -top-20 -left-20 w-64 h-64 bg-primary/30 rounded-full blur-3xl pointer-events-none" />
 <div className="absolute -bottom-20 -right-20 w-64 h-64 bg-primary/30 rounded-full blur-3xl pointer-events-none" />
 ...
 NEON MARBLE RACE
 <span className="block text-lg font-bold text-navy mt-2 tracking-widest opacity-80">GOD MODE EDITION</span>
 </h2>
 ...
 className="text-xs bg-canvas-dark hover:bg-canvas-deeper text-navy px-3 py-1 rounded-lg flex items-center gap-1 transition-colors"
 >
 ...
 <p className="text-xs text-muted-blue text-center py-2">
 ...
 <button
 onClick={startGame}
 className="w-full py-5 bg-gradient-to-r from-primary to-primary/80 text-navy font-black text-2xl rounded-xl hover:scale-[1.02] active:scale-[0.98] transition-all shadow-[0_0_30px_rgba(168,85,247,0.4)] flex items-center justify-center gap-3 group"
 >
 ...
 <h1 className="text-5xl font-black text-navy drop-shadow-[0_0_20px_rgba(255,255,255,0.8)] tracking-tight">
 ...
 className="mt-4 inline-block px-8 py-3 bg-gradient-to-r from-red-600 to-primary/80 text-navy font-black text-xl rounded-full shadow-[0_0_30px_rgba(220,38,38,0.6)]"
 >
 RACE FINISHED!
 </motion.div>
 )}
 ...
 <canvas
 ref={canvasRef}
 className="w-full h-[600px] md:h-[800px] bg-[#0a0a0a] rounded-3xl border-4 border-canvas shadow-2xl cursor-crosshair"
 />

 {/* Live Leaderboard */}
 <div className="absolute top-24 right-6 w-80 bg-electric/80 backdrop-blur-xl border border-white/10 rounded-2xl p-6 pointer-events-none shadow-2xl">
 <h3 className="text-navy font-black mb-4 flex items-center gap-2 text-lg">
 ...
 {ball.rank === 1 && <Sparkles className="w-4 h-4 text-primary ml-auto animate-pulse" />}
 </motion.div>
 );
 })}
 </AnimatePresence>
 {winners.length === 0 && (
 <div className="text-faint-blue text-sm text-center py-8 italic">
 Racing in progress...
 </div>
 )}
 </div>
