...
const CurrencyTicker = () => (
 <div className="w-full overflow-hidden bg-electric border-y border-primary/20 py-3 mb-8 relative">
 ...
 <div key={i} className="flex items-center gap-3 text-sm font-medium text-faint-blue">
 <span className="text-xl grayscale opacity-70">{c.flag}</span>
 <span className="font-serif text-primary/80">{c.id}</span>
 <span className="text-navy font-bold tracking-wider">{c.rate.toLocaleString('ko-KR')}</span>
...
export default function ExchangeRateImpactCalculator() {
 ...
 const inputStyle = "w-full p-4 mt-1 bg-white border border-canvas rounded-xl focus:ring-1 focus:ring-primary/50 focus:border-primary/50 transition disabled:opacity-50 text-white font-medium";
 const textInputStyle = `${inputStyle} text-center font-mono text-lg`;

 return (
 <div className="animate-fade-in-up bg-electric min-h-screen text-white">
 <CurrencyTicker />
 ...
 <p className="text-faint-blue text-lg max-w-2xl mx-auto font-light">
 초고액 자산가를 위한 실시간 환율 영향 분석 및 자산 가치 시뮬레이션
 </p>
 </div>

 <div className="bg-canvas border border-canvas p-8 sm:p-10 rounded-[2rem] shadow-2xl relative overflow-hidden" ref={reportRef}>
 {/* Decorative Elements */}
 ...
 <div className="bg-white/50 p-8 rounded-3xl border border-canvas/50 backdrop-blur-sm">
 <CurrencyInput
 label="분석할 자산 규모"
 value={assetAmount}
 ...
 <div className="space-y-4 p-6 border border-canvas rounded-3xl bg-white/30">
 <div className="flex items-center justify-between">
 <label className="text-xs font-bold text-faint-blue uppercase tracking-wider">Exchange Rate ({comparisonCurrency} → {assetCurrency})</label>
 ...
 <div className={`w-3 h-3 rounded-full border ${isManual ? 'bg-primary/50 border-primary' : 'border-canvas'}`} />
 </div>
 </div>
 <div className="flex items-center gap-4">
 <div className="flex-1">
 <span className="text-xs text-faint-blue block mb-1 text-center">PAST</span>
 ...
 <div className="flex-1">
 <span className="text-xs text-faint-blue block mb-1 text-center">CURRENT (LIVE)</span>
 ...
 <p className="font-serif text-xl text-muted-blue">Analyzing Global Markets...</p>
 </div>
 ) : error ? (
 <div className="flex items-center justify-center h-full p-8 bg-canvas-deeper/10 rounded-3xl border border-canvas/30">
 <div className="text-center">
 <AlertCircle className="w-12 h-12 text-electric mx-auto mb-4" />
 <h3 className="font-bold text-electric mb-2">Analysis Failed</h3>
 <p className="text-sm text-electric/70">{error}</p>
 </div>
 </div>
 ) : (
 ...
 <p className="font-serif text-faint-blue mb-6 tracking-wide uppercase text-xs">
 Net Asset Value Impact
 </p>

 <div className="flex flex-col items-center justify-center gap-2 mb-6">
 <span className="text-xl font-medium text-muted-blue line-through decoration-zinc-600/50">
 ...
 <div className="grid grid-cols-2 gap-6">
 <div className="bg-white p-6 rounded-3xl border border-canvas flex flex-col items-center justify-center text-center hover:border-primary/30 transition-colors group">
 <div className="w-12 h-12 rounded-2xl bg-canvas-dark flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
 <Gem className="w-6 h-6 text-[rgba(255,255,255,0.8)]" />
 </div>
 <p className="text-[10px] font-bold text-faint-blue uppercase tracking-widest mb-1">Luxury Index</p>
 <p className="text-2xl font-bold text-navy">
 {purchasingPower.bigMacs.toLocaleString('ko-KR')} <span className="text-sm font-normal text-muted-blue">Units</span>
 </p>
 </div>
 <div className="bg-white p-6 rounded-3xl border border-canvas flex flex-col items-center justify-center text-center hover:border-primary/30 transition-colors group">
 <div className="w-12 h-12 rounded-2xl bg-canvas-dark flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
 ...
 <p className="text-[10px] font-bold text-faint-blue uppercase tracking-widest mb-1">Lifestyle Index</p>
 <p className="text-2xl font-bold text-navy">
 {purchasingPower.coffees.toLocaleString('ko-KR')} <span className="text-sm font-normal text-muted-blue">Cups</span>
 </p>
 </div>
 </div>

 {/* Chart */}
 <div className="h-48 bg-white p-6 rounded-3xl border border-canvas">
 ...
 <div className="mt-8 pt-8 border-t border-canvas grid grid-cols-1 sm:grid-cols-3 gap-4">
 <button onClick={handleReset} className="w-full py-4 bg-white text-faint-blue font-bold rounded-2xl hover:bg-canvas-dark hover:text-navy transition-all flex items-center justify-center gap-2 border border-canvas">
 <RotateCcw size={18} /> RESET
 </button>
 <button onClick={handleShareLink} className="w-full py-4 bg-white text-faint-blue font-bold rounded-2xl hover:bg-canvas-dark hover:text-navy transition-all flex items-center justify-center gap-2 border border-canvas">
 <LinkIcon size={18} /> SHARE LINK
 </button>
 <button onClick={handleShareImage} className="w-full py-4 bg-white text-faint-blue font-bold rounded-2xl hover:bg-canvas-dark hover:text-navy transition-all flex items-center justify-center gap-2 border border-canvas">
