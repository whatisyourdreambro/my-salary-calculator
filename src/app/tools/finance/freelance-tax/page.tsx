      ...
      <div className="max-w-3xl mx-auto">
        ...
        <motion.div key={r.totalTax} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="bg-primary p-8 text-center">
            <p className="text-slate-900/70 text-xs font-black uppercase tracking-widest mb-2">실수령 순이익 (세후)</p>
            <p className="text-5xl font-black text-slate-900 tracking-tight">{fmt(r.netIncome2)}<span className="text-2xl ml-1">원</span></p>
            <div className="flex justify-center gap-6 mt-5 pt-5 border-t border-white/20">
              <div className="text-center"><p className="text-slate-900/60 text-xs mb-1">순이익(세전)</p><p className="text-slate-900 font-black">{fmt(r.netIncome)}원</p></div>
              <div className="w-px bg-white/20" />
              <div className="text-center"><p className="text-slate-900/60 text-xs mb-1">세금 합계</p><p className="text-slate-900 font-black">{fmt(r.totalTax)}원</p></div>
              <div className="w-px bg-white/20" />
              <div className="text-center"><p className="text-slate-900/60 text-xs mb-1">실효세율</p><p className="text-slate-900 font-black">{r.effectiveRate.toFixed(1)}%</p></div>
            </div>
          </div>
