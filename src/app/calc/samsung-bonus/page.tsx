...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    answer:
      "네, '내 연봉으로 계산' 섹션 안의 '세금 계산 가정 조정'에서 (1) 세액공제율을 0~50%까지 슬라이더로 조정 (디폴트 30%, 자녀·연금·의료비·기부 등 공제 반영 비율), (2) 4대보험 추가 부과 적용 여부를 체크박스로 ON/OFF 할 수 있습니다. 성과급은 보수에 합산되어 4대보험 정산되지만, 국민연금은 보수월액 상한(연 7,404만원)이 있어 고소득자는 추가 부과액이 적습니다.",
  },
  {
    question: "사업부 가중치는 무엇인가요?",
    answer:
      "사업부 가중치는 사업부의 영업이익 기여도·전략적 중요도를 반영한 상대값입니다. 1.0이 표준이고, 0.7이면 70% 가중, 0.0이면 사업부 분배에서 제외(부문 균등 분배만 받음)됩니다. 가중치의 절대 합이 1일 필요는 없고, 사업부 풀(전체 60%)을 Σ(인원×가중치)로 정규화해 분배합니다. 본 계산기 디폴트는 메모리 1.0 / 공통 0.7 / 파운드리·시스템LSI 0.0이며 사용자가 자유롭게 조정 가능합니다.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "성과급은 별도 분리과세가 아니라 연간 근로소득에 합산되어 누진세율(6~45%)이 적용됩니다. 여기에 지방소득세(소득세의 10%), 그리고 4대보험은 보수에 합산되어 정산되지만 국민연금은 보수월액 상한(연 7,404만원) 적용, 건강·고용보험은 상한 없음으로 처리됩니다. 일반적으로 성과급 실효세율은 20~38% 수준이며, 세액공제 활용도와 적용되는 4대보험 부과 방식에 따라 차이가 큽니다.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "SK하이닉스는 PS(Profit Sharing)를 기본급의 일정 배수로 지급하는 방식이고, 삼성전자는 영업이익 풀을 부문·사업부로 분배하는 방식으로 알려져 있습니다. 산정 기준이 달라 직접 비교가 어렵지만, 본 계산기 참고 박스에 SK하이닉스 단순 추정치(영업이익 250조 × 10% ÷ 35,000명 ≈ 7.14억)를 함께 제공합니다.",
  },
  {
...
const DIVISION_SCENARIOS = [
  ...
  },
  {
    ...
    color: "#F59E0B",
    note: "특정 사업부에 속하지 않는 인력. 디폴트 가중치 0.7.",
  },
  {
    name: "파운드리·시스템LSI",
    profitRange: "적자 ~ 1~3조",
          ...
          <article className="prose prose-sm sm:prose-base dark:prose-invert max-w-none mb-10">
            ...
            <ul className="space-y-1 text-muted-blue dark:text-canvas-300 leading-relaxed">
              ...
              <li>
                ...
                토글로 ON/OFF 가능. 국민연금은 보수월액 상한(연 7,404만원) 적용
              </li>
              <li>
