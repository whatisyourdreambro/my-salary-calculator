...
// 12.31 체크리스트(2026-09-25 IRP 계산기로 교체 — 아래). 공무원 2027·4대보험 2027·재산세 푸터는 OCT 세트 그대로 이월.
...
// ('12/31 전 마지막 점검')와 역할이 겹친다. 재산세 푸터는 12/1~15 종부세 납부와 겹쳐 유지.
// 연금계좌 한도 900만(연금저축 600만 포함)은 소득세법 제59조의3·국세청 안내 확인.
// ★푸터 사실(2026-09-25 정정 — 종전 주석의 '푸터(order 21)로 계속 도달'은 틀렸다): 58b8876d 까지 DEC 의
// 체크리스트는 이 상단 블록의 header 전용 항목이라 dedupByHref 가 SEASON_REST 의 푸터 항목(order 21)을
// 가려 DEC 푸터에 없었다(DEC 푸터 27개·헤더 12개). 교체 후에는 헤더에서 빠지는 대신 가려졌던 order 21 이
// 살아나 DEC 푸터에 새로 들어간다(DEC 푸터 28개·헤더 12개 — OCT·JAN 과 같은 28개). 즉 헤더→푸터 이동이다.
