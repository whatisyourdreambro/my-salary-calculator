...
`
      : "";
  // 헤드라인 산정 기준 표기 (A19, 2026-09-25) — b="r" 이면 회사 공시 1인평균급여액(인원 가중) 기준.
  // 임베드 iframe 높이 고정(420px)이라 종전 문구보다 짧게 유지.
  const basisNote = d.b === "r" ? "1인평균급여액 기준" : "급여총액÷인원 기준";
  return widgetShell({
...
${tenureRow}  <p class="note">금융감독원 DART ${esc(d.y)}년 사업보고서 &ldquo;직원 등의 현황&rdquo; ${basisNote}(등기임원 제외, 100만원 단위 반올림). 데이터 기준일 ${DART_INJECTION_DATE}.</p>`,
