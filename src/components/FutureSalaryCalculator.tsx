...
export default function FutureSalaryCalculator() {
 ...
 const marketData = salaryData[marketDataKey] ?? salaryData["all-all-all-all"];
 return calculateFutureSalary(salary, years, baseRate, events, marketData);
 }
 return [];
 }, [mode, currentSalary, years, baseRate, events, jobCategory, experienceLevel]);
 ...
 const updatedData: StoredFinancialData = { ...existingData, futureSalary: futureDataToStore, lastUpdated: new Date().toISOString() };
 ...
 setEvents([...events, { yearIndex: newYearIndex, type: "promotion", value: 10 }]);
 };

 const updateEvent = (index: number, field: keyof SalaryEvent, value: string | number) => {
 const newEvents = [...events];
 const targetEvent = { ...newEvents[index] };

 if (field === "yearIndex") {
 const newYearIndex = Number(value);
 const isYearTaken = events.some((event, i) => i !== index && event.yearIndex === newYearIndex);
 if (isYearTaken) {
 ...
 mode === "predict" ? "bg-background shadow-sm text-primary" : "text-muted-foreground hover:bg-secondary/60"
 }`}
 >
 미래 연봉 예측
 </button>
 ...
 className={`flex-1 px-4 py-2 rounded-md text-sm font-semibold transition-all ${
 mode === "goal" ? "bg-background shadow-sm text-primary" : "text-muted-foreground hover:bg-secondary/60"
 }`}
 >
 목표 달성 경로
 </button>
 ...
 <CurrencyInput label="현재 연봉" value={currentSalary} onValueChange={setCurrentSalary} quickAmounts={[10000000, 1000000]} />
 <NumberStepper label="기간 (년)" value={years} onValueChange={handleYearsChange} min={1} max={30} unit="년" />
 </div>
 </div>
 ...
 <div>
 <label className="text-sm font-medium text-muted-foreground">직군</label>
 <select value={jobCategory} onChange={(e) => setJobCategory(e.target.value)} className={inputStyle}>
 ...
 <select value={experienceLevel} onChange={(e) => setExperienceLevel(e.target.value)} className={inputStyle}>
 ...
 <NumberInput type="number" inputMode="numeric" value={baseRate} onChange={(e) => setBaseRate(Number(e.target.value))} className={inputStyle} />
 </div>
 ...
 <select value={event.yearIndex} onChange={(e) => updateEvent(index, "yearIndex", Number(e.target.value))} className={`${inputStyle} !mt-0 !p-2`}>
 {Array.from({ length: years }, (_, i) => <option key={i} value={i}>{i + 1}년차</option>)}
 </select>
 <button onClick={() => removeEvent(index)} className="text-muted-foreground hover:text-destructive p-1">
 <X size={16} />
 </button>
 </div>
 <div className="flex gap-2">
 <select value={event.type} onChange={(e) => updateEvent(index, "type", e.target.value)} className={`${inputStyle} !mt-0 !p-2 w-1/2`}>
 <option value="promotion">승진</option>
 <option value="job_change">이직</option>
 </select>
 <div className="w-1/2 relative">
 <NumberInput type="number" inputMode="numeric" value={event.value} onChange={(e) => updateEvent(index, "value", e.target.value)} className={`${inputStyle} !mt-0 !p-2 h-full pr-8`} />
 ...
 <button onClick={addEvent} className="w-full mt-4 p-2 bg-primary/10 text-primary font-semibold rounded-lg hover:bg-primary/20 transition">
 + 이벤트 추가
 </button>
 </div>
 )}
 ...
 <CurrencyInput label="목표 연봉" value={targetSalary} onValueChange={setTargetSalary} quickAmounts={[100000000, 50000000]} />
 </div>
 )}
 </div>
 ...
 {mode === "predict" ? "커리어 로드맵 시뮬레이션" : `연봉 ${formatNumber(Number(targetSalary.replace(/,/g, "")))}원 달성 경로`}
 </h2>
 ...
 <Line type="monotone" dataKey="marketAverage" name="시장 평균" stroke="hsl(var(--accent-foreground))" strokeWidth={2} strokeDasharray="5 5" dot={false} />
 <Line type="monotone" dataKey="marketTop10" name="시장 상위 10%" stroke="hsl(var(--primary))" strokeOpacity={0.5} strokeWidth={2} strokeDasharray="5 5" dot={false} />
 </LineChart>
 </ResponsiveContainer>
 ...
 <th className="p-2 text-right font-semibold">나의 예상 연봉</th>
 ...
 <td className="p-2 text-right text-primary font-bold">{formatNumber(item.salary)} 원</td>
 <td className="p-2 text-right text-primary/80">+ {formatNumber(item.increaseAmount)} 원</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 <div className="mt-6">
 <button onClick={handleSaveData} className="w-full py-3 bg-primary text-primary-foreground font-bold rounded-lg hover:brightness-95 transition-all">
 대시보드에 저장
 </button>
 </div>
 </>
 ...
 <div key={path.scenario} className="p-6 bg-secondary/50 rounded-lg border border-border">
 <h3 className="text-xl font-bold text-primary">{path.scenario}</h3>
 <p className="mt-2 text-muted-foreground">{path.description}</p>
 <div className="mt-4 space-y-2">
 {path.events.map((event) => (
 <div key={event.yearIndex} className="p-3 bg-card rounded-md border border-border">
 <p className="font-semibold">{event.yearIndex + 1}년차: {event.type === "promotion" ? "승진" : "이직"}</p>
 <p className="text-sm text-muted-foreground">
 {event.type === "promotion" ? `연봉 ${event.value}% 추가 상승` : `연봉 ${formatNumber(event.value)}원으로 점프`}
 </p>
 </div>
 ))}
 <div className="p-3 bg-card rounded-md border border-border">
 <p className="font-semibold">이후 기간</p>
 <p className="text-sm text-muted-foreground">연 평균 <span className="font-bold text-primary">{path.avgIncreaseRate}%</span>의 연봉 상승 필요</p>
 </div>
 </div>
 <div className="mt-4 p-4 bg-primary/10 rounded-lg">
 <h4 className="font-bold">⭐ 역량 제안</h4>
 <p className="text-sm mt-1 text-primary/90">
 {path.scenario === "안정 성장형" ? "성공적인 승진을 위해 프로젝트 관리, 팀 리더십 역량을 키워보세요." : "성공적인 이직을 위해 최신 기술 스택(AI, 클라우드)을 학습하고 포트폴리오를 준비하세요."}
 </p>
 </div>
 </div>
 ))
 ) : (
 <p className="text-center text-muted-foreground py-10">현실적인 경로를 찾을 수 없습니다. 목표를 재설정해주세요.</p>
 )}
 </div>
 )}
 </div>
