...
//
// 2027 대표 호봉 예상 봉급 표 (서버 컴포넌트) — /teacher-pay-2027·/police-pay-2027·/firefighter-pay-2027 공용.
// 행은 src/lib/payForecast2027.ts 가 만든다(2026 확정 × 예산안 3.9%, 천원 반올림). 전체 호봉표가 아니라
// 대표 몇 줄이며, 예상 열 머리글에 '예상'을 붙여 확정 봉급표처럼 읽히지 않게 한다.
...
export default function PayForecastTable({ caption, rows, firstColumn, regionLabel }: PayForecastTableProps) {
  return (
      ...
      <table className="w-full text-sm tabular-nums min-w-[520px]">
        ...
        <thead>
          <tr className="border-b-2 border-canvas-200 text-navy">
            ...
            <th scope="col" className="py-3 px-2 text-right font-black whitespace-nowrap">2027 예상</th>
            <th scope="col" className="py-3 px-2 text-right font-black whitespace-nowrap">월 차이(예상)</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row) => (
            <tr key={row.label} className="border-b border-canvas-100">
              ...
              <td className="py-2.5 px-2 text-right font-bold text-navy whitespace-nowrap">{fmt(row.predicted2027)}</td>
