...
export default function SalarySlipPage() {
    ...
    const handleShare = () => {
        if (navigator.share) {
            navigator.share({
                title: '???îÍ∏â ?ÅÏàòÏ¶?,
                text: `?∞Î¥â ${salaryInput}?êÏùò ?§Ïàò?πÏï°?Ä ?ºÎßà?ºÍπå?? #Moneysalary #?îÍ∏â?ÅÏàòÏ¶?,
                url: window.location.href,
            });
        } else {
            alert("Í≥µÏú†?òÍ∏∞ Í∏∞Îä•??ÏßÄ?êÎêòÏßÄ ?äÎäî Î∏åÎùº?∞Ï??ÖÎãà??");
        }
    };
        ...
        <main className="w-full min-h-screen bg-zinc-950 flex flex-col items-center pt-28 px-4 pb-20 font-sans text-zinc-100">
            ...
            <div className="text-center mb-12">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-zinc-900 shadow-sm border border-zinc-800 text-zinc-400 font-medium text-sm mb-4">
                    ...
                    <span>?îÍ∏â ?ÅÏàòÏ¶?Î∞úÍ∏âÍ∏?/span>
                </div>
                <h1 className="text-3xl sm:text-5xl font-black text-white mb-4 tracking-tight">
                    ???îÍ∏â, <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-cyan-400">?ÅÏàòÏ¶?/span>?ºÎ°ú ?ïÏù∏?òÍ∏∞
                </h1>
                <p className="text-zinc-400 text-lg">
                    ?åÏÇ¨Î™? ?¥Î¶Ñ ?±ÏùÑ ?ÖÎ†•?òÏó¨ ?òÎßå???ÅÏàòÏ¶ùÏùÑ ÎßåÎì§?¥Î≥¥?∏Ïöî.
                </p>
            </div>
