...
export default function BonusCalculatorPage() {
 ...
 <p className="text-faint-blue font-medium text-lg">
 ...
 : "bg-white border-canvas text-muted-blue hover:border-primary hover:bg-primary/5"
 }`}
 >
 <p>{p.label}</p>
 <p className={`mt-1 font-normal ${salary === p.salary && bonus === p.bonus ? "text-white/70" : "text-faint-blue"}`}>
 ...
 {/* Input Card */}
 <div className="bg-white border border-canvas rounded-2xl p-8 mb-6 shadow-sm">
 <h2 className="text-sm font-black text-faint-blue uppercase tracking-widest mb-6">입력 정보</h2>
 <div className="space-y-6">
 <div>
 <label className="text-xs font-bold text-faint-blue uppercase tracking-widest block mb-2">연간 기본급 (세전, 원)</label>
 ...
 className="w-full border border-canvas rounded-xl px-4 py-3.5 text-xl font-black text-navy focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none pr-16"
 />
 <span className="absolute right-4 top-1/2 -translate-y-1/2 text-faint-blue font-bold text-sm">원</span>
 </div>
 <p className="text-xs text-faint-blue mt-1.5">{fmt(salary)}원 ({(salary / 10000).toLocaleString('ko-KR')}만원)</p>
 </div>

 <div>
 <label className="text-xs font-bold text-faint-blue uppercase tracking-widest block mb-2">성과급 / 인센티브 (세전, 원)</label>
 <div className="relative">
 <input
 type="text"
 inputMode="numeric"
 ...
 className="w-full border border-canvas rounded-xl px-4 py-3.5 text-xl font-black text-primary focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none pr-16"
 />
 <span className="absolute right-4 top-1/2 -translate-y-1/2 text-faint-blue font-bold text-sm">원</span>
 </div>
 <p className="text-xs text-faint-blue mt-1.5">{fmt(bonus)}원 ({(bonus / 10000).toLocaleString('ko-KR')}만원)</p>
 </div>
 ...
 className="w-full border border-canvas rounded-xl px-4 py-3.5 font-bold text-navy focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none"
 >
 {[0,1,2,3,4,5].map(n => (
 ...
 <div>
 <label className="text-xs font-bold text-faint-blue uppercase tracking-widest block mb-2">배우자 유무</label>
 ...
 isSingle === opt.v
 ? "bg-primary text-white border-primary"
 : "border-canvas text-muted-blue hover:border-primary"
 }`}
 >
 {opt.label}
 </button>
 ...
 <div key={item.label} className={`flex justify-between items-center py-2 ${item.highlight ? "border-t-2 border-primary pt-4" : "border-b border-canvas"}`}>
 <span className={`text-sm font-medium ${item.highlight ? "font-black text-navy" : "text-faint-blue"}`}>
 {item.label}
 </span>
 <span className={`font-black tabular-nums ${
 item.highlight ? "text-primary text-xl" :
 item.value < 0 ? "text-electric" : "text-navy"
 }`}>
 ...
 className="w-full flex items-center justify-between p-5 border border-canvas rounded-2xl mb-6 hover:bg-canvas transition-colors"
 >
 <span className="font-bold text-muted-blue text-sm">세금 계산 상세 내역 보기</span>
 {showDetail ? <ChevronUp size={18} className="text-primary" /> : <ChevronDown size={18} className="text-faint-blue" />}
 </button>
 ...
 <div className="bg-canvas px-6 py-4 border-b border-canvas">
 <h3 className="font-black text-navy text-sm">2026 근로소득세 계산 과정</h3>
 </div>
 ...
 <p className="text-xs text-faint-blue mt-0.5">{row.sub}</p>
 </div>
 <p className="text-sm font-black text-muted-blue tabular-nums text-right">{row.value}</p>
 </div>
 ))}
 </div>
 </div>
 ...
 {/* Related */}
 <div className="mt-10">
 <h2 className="text-sm font-black text-faint-blue uppercase tracking-widest mb-4">관련 계산기</h2>
 ...
 <a key={l.href} href={l.href} className="flex items-center justify-between p-4 border border-canvas rounded-xl hover:border-primary hover:bg-primary/5 transition-all group">
 <span className="text-sm font-bold text-muted-blue group-hover:text-primary transition-colors">{l.label}</span>
 <ArrowRight size={14} className="text-faint-blue group-hover:text-primary transition-colors" />
 </a>
 ))}
 </div>
 </div>
