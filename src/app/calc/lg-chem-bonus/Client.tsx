...
export default function LgChemBonusClient() {
  ...
  const calc = useMemo(() => {
    ...
    const estimatedAnnualSalary = monthlyBasicWon * 17;
    const psPct = customMode ? customPsPercent : scenario.percent;

    const psWon = monthlyBasicWon * (psPct / 100);
    const piWon = includePi ? monthlyBasicWon * (PI_FIXED_PERCENT / 100) : 0;
    const totalGross = psWon + piWon;
  ...
  }, [scenarioId, monthlyBasicManwon, includePi, customMode, customPsPercent, creditRate, applyInsurance, scenario]);

  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-canvas-deep bg-white p-6 sm:p-8">
        <h2 className="text-xl font-black mb-4">1단계 · PS (사업부 시나리오)</h2>
      ...
      <section className="rounded-2xl border border-canvas-deep bg-white p-6 sm:p-8">
        ...
        <p className="mt-2 text-xs text-faint">추정 연봉(기본급 × 17) = <strong>{fmtEok(calc.estimatedAnnualSalary)}</strong></p>
      </section>
      ...
      <section className="rounded-2xl border border-canvas-deep bg-white p-6 sm:p-8">
        ...
        <label className="flex items-center gap-3 p-4 rounded-xl border border-canvas-deep cursor-pointer hover:bg-canvas/30">
          ...
          />
          <div>
            <p className="font-bold">PI 포함 (연간 고정 200%)</p>
            <p className="text-xs text-faint mt-0.5">
              회사 적자여도 안정 지급 (노사 합의로 변경 가능성 있음).
              포함 시 {fmtManwon(monthlyBasicManwon * 10000 * 2)} 추가.
            </p>
          </div>
        </label>
      </section>
      ...
      <section className="rounded-2xl border-2 border-primary bg-gradient-to-br from-primary/5 to-primary/10 p-6 sm:p-8">
        ...
        <div className="grid sm:grid-cols-2 gap-3 mb-5">
          <ResultCard label="PS (사업부)" value={fmtManwon(calc.psWon)} sub={`기본급 × ${customMode ? customPsPercent : scenario.percent}%`} />
          <ResultCard label="PI (고정)" value={fmtManwon(calc.piWon)} sub={includePi ? "연간 고정 200%" : "포함 안 함"} />
        </div>
        <div className="rounded-xl bg-white border border-primary/30 p-5">
          <p className="text-xs font-bold text-faint mb-1">📊 PS + PI (세전)</p>
