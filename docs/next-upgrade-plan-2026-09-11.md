...
| **10/1(목) 09:00 KST 이후** | **L10' 회사 description 켜는 예약 재빌드**(9/25 추가·9/25 수정 — 운영자 승인 적용일 10/1, 승인⑧·A7'·A4'·COMP-06) — 새 description·대상 164곳 수정일 승격·A4' 공시 카드 출처 라벨·DART 주입 출처 꼬리(' — OpenDART 수집') 생략(카드·FAQ·FAQPage JSON-LD)은 모두 빌드 시점 KST 날짜로 켜진다(`src/config/siteDates.ts` `COMPANY_META_DISCLOSED_DATE` = 2026-10-01). 그 전 빌드(9/26 시즌 재빌드 포함)는 description·공시 카드·FAQ·수정일이 병합 전 main(bd68d860)과 바이트 동일(테스트 고정본 대조)이라 먼저 합쳐져도 무해. **09:00 KST 이후**인 이유: 수정일이 UTC 자정(2026-10-01T00:00Z = 09:00 KST)으로 나가 그 전 빌드면 lastmod 가 미래가 되고, 자정을 가로지르는 빌드는 일부 페이지만 켜진다. 아래 보유세 재빌드와 **같은 빌드 1회**로 처리 | 빌드+콘솔 | ① 10/1 09:00 KST 이후 main 푸시 또는 CF Pages 'Retry deployment' ② **운영자 Purge**(Retry 로 재빌드했으면 수동) ③ IndexNow 는 postbuild 가 sitemap lastmod 차이로 대상 164 URL 만 자동 제출 — CF 빌드 로그 `[indexnow]` 확인 ④ 확인: 운영 `/salary-db/krafton` meta·og description 끝이 `공시 평균연봉 1억 2,900만원(2025 사업연도).`, 공시 카드 출처 줄 `출처(공시 원문):` 있고 `OpenDART 수집` 0건, `/salary-db/samsung-electronics` 는 `로그인 없이` 있고 `업데이트` 0건, `/sitemap.xml` krafton lastmod 2026-10-01, `/rss-companies.xml` 첫 item pubDate `Thu, 01 Oct 2026` ⑤ **10/5 서치어드바이저 RSS 2종 상태 확인** — 첫 제출이 아니다. rss.xml·rss-companies.xml 은 **8/15 부터 등록돼 있다**(등록 시각 8/15 23:17:44·23:17:55, `docs/revenue-audit-2026-09-08/live-naver-audit.md` 제출 화면 등록 목록). 10/5 에는 두 피드의 마지막 수집 시각·오류만 보고, 같은 주소를 다시 제출하지 않는다(2026-09-26 정정, W1-G) ⑥ 판정 = **GA4 네이버 유입 랜딩 세션 비교**(운영자 2026-09-26 승인 9번·NAVER-02 — GSC-KR 28일 페이지 목록에는 클릭 있는 회사 페이지가 없어 GSC 로는 판정할 수 없다): 처치 164곳(L10' 꼬리) vs 대조 266곳, 창 9/3~9/30 vs 10/2~10/29, **10/30 조회**. 회사별 증감의 중앙값으로 보고 뉴스 급등 회사·별칭 변경 회사(W3-F)는 뺀다. 절차 `docs/analytics-measurement.md` 'L10' 판정 — GA4 네이버 유입 비교'. 첫 빌드가 10/5 뒤로 밀리면 상수를 실제 배포일로 바꾼 뒤 배포 ⑦ 켜진 배포 확인 뒤 `seo.ts` 적용 전 분기·카드/FAQ 게이트·`fixtures/companyPreGate-bd68d860.json` 정리 |
...
| 10/2 조회 | **GA4 `nav_type` 탐색 — 링크 이동 광고 회수 2단계 결정 입력**(9/25 추가, 수익 추천 #1 1단계 N2 `28efd39f`) | GA4 탐색(맞춤 측정기준 `nav_type`, 이벤트 범위) | 창 = 라운드 2 배포 익일~10/1(완료일만). 템플릿별 `R_landing`·`R_soft`(= ad_request_attempt ÷ page_view)와 soft 비중. **회사 DB `^/salary-db/` 에서 `R_soft < 0.5 × R_landing` 이면** 10/12 '회사 DB 본문 링크 하드 이동' 시험을 운영자 승인 항목으로 상정(광고량이 바뀌는 변경 — CF 캐시 HIT 확인 먼저), `≥ 0.5` 면 진행하지 않음, landing·soft page_view 중 하나라도 500 미만이면 10/9 조회(~10/8)로 미룸. 애드센스 착지 대조(/salary-db·전체)를 같은 줄에 기록. 전제: 배포 당일 맞춤 측정기준 등록(소급 없음). 절차·기록표 `docs/analytics-measurement.md` '10/2 탐색'·'10/12 2단계 시험 판정 규칙' |
...
| 10/5 | ~~앵커 판정~~ → **처치 없음, 28일 광고 형식 스냅샷**(9/25 정정) | AdSense | 판정 아님. 기록만(앵커·인페이지·전면·사이드레일 행). /home-loan S2-5 는 기존대로 '참고' 판정. ★2026-09-26(운영자 승인 7번·NAVER-06): 같은 세션에서 /home-loan 의 네이버 웹 탭 순위를 '주담대 금리 계산기'·'주택담보대출 계산기'·'대출한도 계산'으로 확인 — 한 곳이라도 10위 안이면 **10/6~10/7 제목·설명 1회 교정**(브랜치 `claude/r3-w1f-20260926`), 모두 10위 밖이면 건너뜀(제목으로 순위를 못 고침) |
| 10/12 | 홈 설명문 정확성 1회 교정(2026-09-26 추가, 운영자 승인 8번·NAVER-09) | 코드 | `HOME_META_DESCRIPTION` 의 '연간 세액 추정을 월로 환산' → '근로소득 간이세액표를 적용한'(같은 15자, 9/25 A17 간이세액표 전환과 일치). 홈 title 은 브랜드 그대로 — A30 종료. 브랜치 `claude/r3-w3g1-20260926` |
...
| 10/17 | /calc/hyundai-bonus 9/19 변경분 D+28 | 네이버 서치어드바이저 | 기존(마스터플랜 §0-α). 이 판정 전 hyundai title·description 변경 금지. ★2026-09-26(운영자 승인 7번): 9/19 변경이 CTR 을 올리지 못했을 때만 **10/18 이후 1회** 제목·설명 교정(W3-G②, 검색어 선두 '현대차 성과급 계산기'·'현대자동차 성과급'). `/police-pay-2026` 은 B20 판정(T0+35) 뒤 **10/31 무렵** 1회 |
...
| ~~통합 배포 T0+28~~ → **통합 배포 T0+35(≈10/30)** | 네이버 저CTR 3쪽 제목 판정(9/25 추가, B20) — **2026-09-26 운영자 승인 9번(NAVER-02)으로 T0+35 로 이동**: 서치어드바이저 자료는 약 1주 늦게 갱신된다 | 네이버 서치어드바이저 페이지별 | `docs/gsc-sniping-log.md` Round 3. 같은 날 L10' GA4 판정(아래 10/30 행) |
| **10/30(금) 조회** | **L10' 회사 설명문 판정(GA4 네이버 유입 비교) · B20 판정(T0+35)**(2026-09-26 추가, 운영자 승인 9번·NAVER-02) | GA4 탐색(방문 페이지 × 세션 소스 'naver' 포함) · 네이버 서치어드바이저 페이지별 | L10': 처치 164 vs 대조 266, 9/3~9/30 vs 10/2~10/29, 회사별 증감 중앙값, 뉴스 급등·별칭 변경(W3-F) 회사 제외 — `docs/analytics-measurement.md` 'L10' 판정'. 결과가 나쁘지 않으면 회사 설명문 2차(승인 6번·NAVER-04, 10/10 에 꺼진 채 합류) 켜기로 넘어간다. 매주 월요일 네이버 상위 30 기록(9/28~, `docs/operator-console-pack.md`)을 시계열 보조로 쓴다 |
...
| S2-4 | ~~회사 title·description~~ → **L10'(승인⑧) 실행 시 흡수** | 회사 title 은 2026-07-06 결정·"절대 불변". description 은 승인⑧(공시 평균 후미 삽입, 9/28 이후)이 같은 430쪽 대기 | L10' 커밋에 'description 150자 상한·핵심 숫자 선두' 규칙 동봉. title 무접촉. 판정 28일 KR 필터. ★9/25 L10' 구현 완료(150자 상한·자체 추정치 수치 선두 유지·공시 평균 후미) — 10/1 KST 예약 재빌드에서 켜짐(§1 10/1 L10' 행, 운영자 승인 적용일) | — |
...
| P-5 | 전환 전 | **운영자 승인**: 전환은 `/salary` 416쪽·`/monthly` 105쪽 제목의 연도 라벨이 전부 2027 로, 금액은 /salary 361쪽·/monthly 95쪽이 1만원 단위로 바뀐다(2026-09-25 추정 — 장기요양·고용 2026 준용 기준). 회사 430쪽 실수령 열·홈 FAQ 금액도 바뀐다 | 홈 `<title>`(`HOME_META_TITLE`)·회사 `<title>`·`/calc/*` 제목은 포인터가 자동으로 바꾸지 않는다 — 본문은 2027 요율로 바뀌므로 연도 라벨은 **P-6 에서 같은 커밋으로** 처리(회사 title 은 불변 원칙이라 제외) |
| P-6 | **전환 커밋과 같은 커밋**(동결기 허용 — 메타·제목·문자열만, URL·라우트 무접촉. 2026-09-25 추가) | **'2026' 제목 아래 2027 요율이 뜨는 쪽 정리** — 아래 쪽은 본문 요율 문장·계산값이 포인터를 따르는데 `<title>`·H1·JSON-LD 가 '2026' 을 박아 두었다(2026-09-25 grep). 한 줄 전환만 하면 '2026 … 계산기' 제목 아래 국민연금 5.0% 등 2027 값이 나온다. ① **홈 `/`**: `src/lib/homeContent.ts` `HOME_META_TITLE`('2026 연봉 계산기 \| 세후 월급·4대보험 공제액 — 머니샐러리') · `src/app/HomeClient.tsx` h1 '2026 연봉 계산기' · `src/app/layout.tsx` 키워드 '2026 연봉 계산기'·OG alt '2026년 연봉 실수령액 계산기' · `src/lib/seo.ts` 기본 키워드 '2026 연봉 계산기'(본문 FAQ·HowTo 의 'YYYY년 예상 월 실수령액'은 이미 파생) ② **`/calc/2026-year`**(연봉 계산기 PRO — 계산기·요율 안내 문장이 포인터): metadata title '2026 연봉 계산기 PRO - 최신 세법·티어 카드·자산 시뮬'·description '2026년 변경 국민연금/건강보험 요율 자동 적용'·키워드 · SoftwareApplication name·description · breadcrumb leafName · h1 '2026년 연봉계산기 PRO' · 배지 '2026년형 업그레이드 완료' — URL `/calc/2026-year` 는 그대로 두고 문자열만(새 라우트·리다이렉트는 동결기 밖) ③ **`/tools/finance/bonus`**(4대보험 = `CURRENT_INSURANCE_RATES`): `layout.tsx` title '성과급·인센티브 세금 계산기 - 2026 연봉합산 세율'·description·JSON-LD description '2026 연봉합산 방식 …'. 소득세는 2026 세율표(`taxConstants2026`) 그대로라 '연봉합산 세율' 문구의 연도는 2027 세율 개정 여부와 함께 정한다 ④ **`buildToolMetadata` 제목 접미사 ' 2026 — '**(`src/lib/seo.ts:230`) 중 포인터 엔진 쪽 5쪽 — `/calc/holiday-bonus`(description '2026년 세법 기준' 은 P-4 목록)·`/calc/offer-compare`(description '2026년 연간 세액 간이 추정')·`/calc/incentive-tax`·`/calc/year-end-bonus`·`/calc/year-end-bonus-tax`. 헬퍼는 21쪽 공용이므로 헬퍼 문자열을 통째로 바꾸지 말고 쪽별 연도 인자로 나눈다 — 2026 귀속 고정 쪽(`/calc/dual-income-year-end`·`/calc/smb-income-tax-break`)과 `/calc/pension-hike-2027` 은 그대로 ⑤ **성과급 계산기 21쪽** `src/app/calc/*-bonus/page.tsx` 의 `PAGE_TITLE` '… 성과급 계산기 2026'(셀트리온·두산에너빌리티·GS칼텍스·한화에어로스페이스·HD현대중공업·현대모비스·현대로템·카카오·한국전력·기아·LG화학·LG디스플레이·LG에너지솔루션·네이버·포스코·S-OIL·삼성바이오로직스·삼성디스플레이·삼성SDI·SK하이닉스·SK이노베이션)과 `/calc/bonus-calculators` '성과급 계산기 2026 — …' — 본문 출처 문장('YYYY 세법 반영')만 포인터라 전환 뒤 '2027 세법 반영'이 '2026' 제목 아래 뜬다. 제목의 2026 을 성과 연도로 읽을 수도 있으므로 2027 로 올릴지·빼거나 둘지를 쪽별로 정한다. 현대차 `/calc/hyundai-bonus`(제목 '현대차 성과급 계산기 2026 — 현금·주식·세금 비교')도 계산값이 `calcBonusNet`(포인터)라 같은 묶음 — 10/17 제목 판정 이력 확인 뒤. `samsung-bonus` 는 P-3 전까지 포인터 밖 | 결정은 P-5 승인과 함께 받는다(홈 제목은 M01 이력·GSC 28일 창 확인 뒤). **바꾸지 않는 것**: 회사 `<title>` '연봉 2026'(`seo.ts:317-320`, 불변 원칙 — 회사 h1·FAQ 질문은 P-4 연도 롤오버 결정) · `/salary-db/ranking` '2026 대기업 연봉 순위 TOP 30'(데이터 연도 — 계산기 카드 '{YYYY} 세법 기준'만 포인터) · `/salary-db/listed/*`(제목이 공시 사업연도) · `/pro/career-planner` 가이드 카드 '2026년 연봉 실수령액표'(링크 대상 `/table/2026` 이 2026 고정) · 이미 파생: `/salary/[amount]`·`/monthly/[amount]` 제목·설명·FAQ·HowTo, 위젯·OG · 영문(`/en` 제목·h1·JSON-LD)은 P-4. 재확인: `grep -n "2026" src/lib/homeContent.ts src/app/HomeClient.tsx src/app/calc/2026-year/page.tsx src/app/tools/finance/bonus/layout.tsx src/app/calc/bonus-calculators/page.tsx src/app/calc/*-bonus/page.tsx` + `grep -rln buildToolMetadata src/app` 를 포인터 엔진(`config/currentRates`·`bonusTaxCalc`·`TaxLogic`·`lib/calculator`) import 와 교차 |
...
2. 같은 커밋의 연도 라벨(P-6 결정분 — 홈·`/calc/2026-year`·`/tools/finance/bonus`·도구 5쪽·성과급 계산기 21쪽과 현대차·허브 제목·H1·JSON-LD 문자열)과 날짜 상수(콘텐츠가 실제로 바뀌므로): `src/config/siteDates.ts` `TAX_TABLE_EFFECTIVE_DATE`·`src/app/sitemap.ts` `SALARY_METHOD_REVIEW_DATE`·`'/'`·`'/about'` override → 전환 커밋일, `src/lib/ogUrlVersion.ts` `OG_URL_VERSION` → `"20270101"`(1만원 단위가 그대로인 저연봉 카드도 새 연도 라벨로 다시 그리게).
...
| A17 계산 방법 문구 — 동결 메타 2건: 홈 `HOME_META_DESCRIPTION`(`src/lib/homeContent.ts`, '연간 세액 추정을 월로 환산') · `/calc/offer-compare` 설명 끝 '2026년 연간 세액 간이 추정.' → 간이세액표 기준 문구로 교체 | 2026-09-25 월 소득세를 근로소득 간이세액표로 전환해 두 설명이 실제 계산과 어긋남. 홈 설명은 운영자 결정, `/calc/*` 설명은 동결(calcDescriptionFreeze) 해제 10/10 이후. ★2026-09-26 홈 설명 **승인(운영자 승인 8번·NAVER-09)** — 같은 길이 문구로 10/12 배포(`claude/r3-w3g1-20260926`), 홈 title 불변. `/calc/offer-compare` 는 그대로 10/10 뒤 대기 |
...
| 19 | 5월 대량 가이드 약 181편 재작성 | A25(EDIT-05) | **3단 재범위 — 지금 진행**(2026-09-26 운영자 승인 13·14번 · GUIDES-01·GUIDES-07) | 한 문장 단위 정정은 ✅(B9 `4c3302d8`·`6bd584a6`·`e73b76d1`·`c1aa72f6`·`142921ee` 등) · 재작성 사양·금지 사실 게이트 `d98fa74a`·`62885345`(W1-E, 테스트 전용) · ① **대표 39편 재작성**(W3-A) ② **약 32~40편 308 통합** — 방문 0·틀린 숫자가 주소에 박힌 글('7월 건보 정산'·'590만'·'난임 20%' 등)을 재작성된 대표 글로. 제휴 슬롯 글·클릭 있는 글 제외 ③ **약 110편 문장 정정만**(F-C) — 구조 재작성은 2/1 뒤. 원안 일정(준비 10/6~10/9 무배포 → 1차 10/13(화) → 308 10/16 → 2차 10/20)은 '지금 진행' 승인으로 준비를 9/26 부터 시작, 배포일은 가이드 워크플로 통합 기록을 따른다. **구조 변경 마감 10/24 유지**(/guides 인덱스는 10/25 부터 광고 7번 A09) | 사실 원장 → 재작성 → 2중 검증. H2 이동은 광고 삽입점 이동이라 ad-depth 전후 필수. 11/1 뒤에는 문자열만. 배포마다 네이버 수집 요청·IndexNow 확인·공변량 기록(GUIDES-12, 콘솔팩 '네이버·GSC 통합 체크리스트') |
| 20 | 홈 제목에 '실수령액' 복원 | A30(G2·NV-12) | **종료(2026-09-26 운영자 승인 8번 · NAVER-09)** | 홈 title 은 브랜드 제목 그대로(네이버 메인 페이지 가이드: 메인 제목 = 브랜드, 잦은 변경 불이익). 설명문만 10/12 1회 정확성 교정(§1 10/12 행, `claude/r3-w3g1-20260926`) | 운영자 9/24 보고서 §4: 홈 네이버 226,007노출·CTR 0.2%. 홈 클릭의 약 90% 가 브랜드 검색('머니샐러리' 374클릭/836노출)이고 '연봉 실수령액 계산기' 웹 탭 약 50위라 제목 교체 이득은 약 0. 재상정하지 않는다 |
| 21 | 퇴직금 계산기 통합·링크 모으기 | A31(G4) | 예정 | **10/11 이후** | 포크 엔진 → 정본(severanceCalculator) 먼저. 헤더 href 는 사이트 전체 변경 |
| 22 | 연말정산 입력칸 광고 위 실험 | A32(G5) | 광고 원장 | `docs/ad-experiments.md` 2026-09-25 절 4 | ★2026-09-26 재범위(운영자 승인 11번·SEASON-02): 10/17 시작, 주 판정창 11/2~11/29 — 광고 원장 22번 |
...
| 26 | OG 이미지 전략 | A24(META-01·OG-01) | 순서 확정 | **캐시 규칙 C(C01) 먼저 → 7일 뒤 유형별 실패율 재측정 → 그다음 빌드 타임 정적 PNG 여부 결정** | B16 OG 신뢰성 수정(`637604ae`·`eed99fef`)은 반영. ★2026-09-26 **조건부 승인(운영자 승인 17번·CRAWL-04)**: 10/2 재측정에서 OG 가 여전히 크롤 서버 오류의 절반 이상이면 회사·비교·상장사·월급·직업 OG 약 1,260장을 빌드 시 정적 PNG 로(빌드 +1~2분, 용량 +60~100MB). 회사 이미지는 10/29 판정 뒤 전환. 24시간 캐시(CRAWL-01)로 충분히 줄면 보류 |
...
| 보고서 P1 | /home-loan 검색어·제목·첫 답변 대조 | S2-5 → NAVER-06(W1-F) | **조건부 승인(2026-09-26 운영자 승인 7번)** | **10/6~10/7**, 10/5 네이버 웹 탭 10위 안일 때만 — 브랜치 `claude/r3-w1f-20260926` | description 실험('참고' 강등) 종료 전 변경 금지. 교정은 title·description 만(둘 다 이전 이하 길이), H1·리드 문단·숫자 무변경이라 광고 위치 동일. 1회만 — T0 는 배포 때 `docs/gsc-sniping-log.md` 에 기입, 판정 T0+28 + 네이버 지연 1주 |
| 보고서 P1 | /calc/hyundai-bonus | S3-6 대체 → NAVER-06(W3-G②) | **조건부 승인(2026-09-26 운영자 승인 7번)** | **10/18 이후**, 10/17 D+28 에서 9/19 변경이 CTR 을 못 올렸을 때만 | 9/19 변경분 D+28. PAGE_TITLE·PAGE_DESC 두 줄만(광고 5번이 같은 page.tsx 결과 영역을 건드렸으면 그 위에 리베이스) |
| 보고서 P1 | /police-pay-2026 | NAVER-06 | **승인(2026-09-26 운영자 승인 7번)** | **10/31 무렵**, B20 판정(T0+35) 뒤 | 10/14 예약 병합(`claude/r2-l2-20260925`)은 이 메타를 건드리지 않음. 소방과 거의 같은 표(별표 10)라 문구는 소방과 다르게 |

## 10. 2026-09-26 승인 원장 — 운영자 답(R3 제시 19문항)

운영자가 2026-09-26 R3 계획(`r3` 준비 브랜치)의 승인 문항에 답했다. 광고에 걸리는 문항(2·3·4·5·11·15·18)은 `docs/ad-experiments.md` 2026-09-25 절 4 아래 '2026-09-26 추가'에도 적었다. 계획 ID 는 R3 계획의 항목 ID 다. 배포는 예약 달력(오케스트레이터)을 따르며, 커밋이 없는 항목은 콘솔·결정 기록이다.

| 번호 | 항목 | 계획 ID | 결정 | 시점·조건 | 기록·브랜치 |
|---|---|---|---|---|---|
| 1 | 예약 배포 시각 09:10 KST | — | 승인 | ~~9/29 09:10~~ 9/26 13:10:50 main → `claude/deploy-0929` 빨리감기(운영자 '메인푸시해' 즉시 배포) · 10/1 09:10 L10' 켜짐 재빌드(보유세 비시즌 전환과 같은 빌드) | 공변량 `docs/ad-experiments.md` 절 3(c) |
| 2 | 착지 뷰 자동광고 관측 `autoads_seen`(aa1)을 1차 배포에 포함(9/26 13:10 배포) | AAS(`claude/r3-aas-20260926`) | 승인 | 9/26 13:10 배포. 판독 10/2(수집 점검)·10/9(판정) | `27db60e9`·`50401ae2`, `docs/analytics-measurement.md` '자동광고 관측' |
| 3 | 삼성 성과급 파일 두 번째 접촉 | RPM-05 | 승인 | 10/12~10/17, 광고 5번(오클릭 방어)과 같은 날 — 결과 아래 링크 대상 DS/DX별 교체(개수·높이 불변) + 1/1 요율 전환 준비(P-3) + 새로고침 이동 플래그(꺼 둠) | — |
| 4 | 헤더·로고·빵부스러기 링크 새로고침 이동 | RPM-04 | **미승인** | 2027-02 구조 슬롯 | 재상정은 10/19 추천 #1 시험 결과·전면광고 판독 뒤 |
| 5 | '전면광고 표시' 계측 이벤트 추가 | RPM-03 | **미승인** | 이벤트 없음. 10/2·10/9 판독은 수동 3줄만 | `docs/analytics-measurement.md` 'RPM-03 전면광고 판독' |
| 6 | 회사 설명문 2차 — 공시 평균이 제목의 시니어 추정보다 높은 111곳 중 절반에 공시 평균 꼬리 | NAVER-04 | 승인(50/50 분할 시험) | 코드는 10/10 에 꺼진 채 합류 → 10/30 L10' 판정이 나쁘지 않으면 켬. 제목 불변, 뉴스 출처 16곳은 DART 대조 | — |
| 7 | 저CTR 스니펫 1회 교정 3쪽 | NAVER-06 | 승인(조건부) | /home-loan 10/6~10/7(10/5 네이버 웹 탭 10위 안일 때만) · /calc/hyundai-bonus 10/18 이후(10/17 D+28 효과 없을 때만) · /police-pay-2026 10/31 무렵(B20 판정 뒤) | §9 보고서 P1 행 · /home-loan `claude/r3-w1f-20260926` |
| 8 | 홈 설명문 같은 길이 정확성 교정 | NAVER-09 | 승인 | 10/12. 홈 title 불변, A30 종료 | §9 20번 · `claude/r3-w3g1-20260926` |
| 9 | L10' 판정을 GA4 네이버 유입 비교로, B20 판정을 T0+35 로 | NAVER-02 | 승인 | 10/30 조회. 매주 월요일 네이버 상위 30 기록 9/28 시작 | §1 10/1·10/30 행 · `docs/analytics-measurement.md` · `docs/gsc-sniping-log.md` Round 3 · 콘솔팩 |
| 10 | 연말정산 약 12쪽 제목 '2027 연말정산(2026년 귀속)' 1회 | SEASON-06 | 승인 | 10/14~10/16, 주소 불변·미리보기 제외. 12/15 값 교체는 폐기(`ea116861` 가드) | `docs/revenue-10x-plan-2026-09.md` M07 |
| 11 | A32 시작 10/17 · 주 판정창 11/2~11/29 | SEASON-02 | 승인 | 표본 미달이면 12/27 까지 연장, 14일 중간 확인 삭제, 원위치 규칙 유지 | 광고 원장 22번 |
| 12 | `_routes.json` 정적 계열 Worker 제외 | NAVER-07 | 미룸(승인) | 2027-02 | 같은 효과는 24시간 캐시(CRAWL-01)로 |
| 13 | 가이드 재작성 | GUIDES-01 | 승인 — 지금 진행 | 즉시 준비, 구조 변경 마감 10/24 | §9 19번 |
| 14 | 방문 0·틀린 숫자 가이드 308 통합 | GUIDES-07 | 승인 — 지금 진행 | 제휴 슬롯 글·클릭 있는 글 제외 | §9 19번 |
| 15 | 간이 계산기 긴 본문 배치 | CALC-03 | **A(현행 유지)** | — | 아래 주석 |
| 16 | 계산기 출처 허용 목록에 kepco.co.kr·opinet.co.kr | CALC-05 | 승인 | 9/26 배포에 포함(`4892b4c3`) | — |
| 17 | 회사·비교·상장사·월급·직업 OG 정적 PNG | CRAWL-04 | 조건부 승인 | 10/2 재측정에서 OG 가 크롤 서버 오류의 절반 이상일 때만. 회사 이미지는 10/29 판정 뒤 | §9 26번 |
| 18 | 애드센스 URL 채널 10개 추가 | RPM-01 ⑤ | ✅ 완료(Claude, 2026-09-26) | 교사·경찰·소방·공무원 2026 봉급표, `/industry/`, `/monthly/`, `/salary/`, `/table/`, 성과급 모음(`/calc/bonus-calculators`), 회사 비교(`/salary-db/compare/`). 소급되지 않으므로 이날부터 집계 | `docs/analytics-measurement.md` '콘솔 변경 기록표' |
| 19 | GA4 `/guides/` 랜딩 90일 소스별 자료 | GUIDES-03 | ✅ 완료 | 재작성·통합 대상 선정 자료(원본은 저장소 밖) | — |

- **15번 주석(CALC-03 결정 기록)**: 간이 계산기 S3-1 긴 본문(`details`, 중앙값 894자)은 explanation 섹션 안, GuideMidAd·InArticleAd 위에 그대로 둔다(A). 103쪽이 9/12~9/14 부터 이 배치로 운영 중이고 수익 영향은 월 ±$0.5 수준으로 봤다. 모바일에서 두 광고가 약 1,200~1,500px(추정) 아래로 밀리는 것은 **승인된 현행 상태**로 기록한다. `src/components/SimpleCalculatorView.tsx:390` 주석의 '광고 이동 없음'은 **순서**에 대해서는 맞지만 **깊이**에 대해서는 틀리다. 주석 자체는 그 파일을 만져도 되는 뒤 배치에서만 고친다(광고 5번 주간에는 손대지 않음). 본문을 여러 `<p>` 로 쪼개지 않는다(자동광고 삽입점이 늘어 광고량 변경이 된다).
- 이 원장의 '미승인' 두 문항은 진행하지 않는다. 4번은 2027-02 구조 슬롯 전에는 재상정하지 않는다. 5번은 이벤트 없이 수동 판독 3줄로 보고, 그래도 결론이 안 나면 그때 자료를 붙여 한 번 더 묻는다(자동 진행 없음).
