// ===================================================================
// 2025년 대한민국 4대 보험 및 소득세 정보 (최종판: 부양가족 반영)
// ===================================================================
...
// 부양가족과 자녀 수를 인자로 받도록 함수 시그니처를 수정합니다.
export function calculateNetSalary(
  annualSalary: number, 
  nonTaxableAmount: number = 0,
  dependents: number = 1, // 본인 포함
  children: number = 0
) {
  if (annualSalary <= 0) {
    return {
  ...
  const taxableMonthlyIncome = Math.max(0, monthlySalary - (actualNonTaxableAmount / 12));
  
  const pension = Math.min(taxableMonthlyIncome * PENSION_RATE, PENSION_MONTHLY_CAP);
  ...
  // [수정] 부양가족 1인당 150만원 인적공제 적용
  const personalDeduction = dependents * 1500000; 
  const pensionDeduction = pension * 12;
  
  const taxBase = Math.max(0, taxableAnnualSalary - earnedIncomeDeduction - personalDeduction - pensionDeduction);
  
  const calculatedTax = getCalculatedTax(taxBase);
  const taxCredit = getTaxCredit(calculatedTax, taxableAnnualSalary);

  // [수정] 자녀 세액공제 적용 (1명 15만원, 2명 30만원 등 단순화)
  const childTaxCredit = children * 150000;
  
  const finalAnnualTax = Math.max(0, calculatedTax - taxCredit - childTaxCredit);
