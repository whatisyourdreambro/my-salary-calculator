...
| 월 | +3분 | ★(2026-09-25 추가) AdSense 보고서 **날짜 × 광고 형식** + 날짜 × 광고 단위: 자동 인페이지 노출/PV = (인페이지 행 − 같은 날 수동 광고 단위 노출 합) ÷ PV 를 지난 7일과 그 전 7일로 비교 — **−30% 이하면 경보** → Claude 에 '자동광고 경로 점검' 요청(`npm run verify:autoads`·운영 HTML 루트 셸 클래스·CF 캐시 잔존 순). 정의·기준값 `docs/ad-experiments.md` 2026-09-25 절 2. 9/10 붕괴(−75%)가 2주간 안 보였던 공백 |
