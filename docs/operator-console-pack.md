...
## 7. 인피드(In-feed) 광고 단위 발급 — R2 A2 (5분, 10월 실험용) ☐
1. adsense.google.com → 광고 → **광고 단위 기준** → **인피드 광고** 만들기
2. 이름 예: "머니샐러리_인피드(랭킹리스트)" → 스타일은 자동 제안 그대로 → 만들기
3. 발급된 **슬롯 ID(숫자)** 를 저에게 알려주시면, 10월 초 실험 슬롯에서
   랭킹·리스트 페이지에 배치합니다 (실험 #1 판정 후 진행 — R2 §2 A2).
