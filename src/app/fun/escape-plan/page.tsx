...
const ProgressBar = ({ percentage }: { percentage: number }) => (
 <div className="w-full bg-canvas-deeper rounded-full h-6 overflow-hidden relative">
 <motion.div
 ...
 <div className="absolute inset-0 flex items-center justify-center text-xs font-bold text-muted-blue mix-blend-difference">
...
export default function EscapePlanPage() {
 ...
 className="bg-gradient-to-br from-[#0145F2] to-[#0D5BFF] text-navy p-8 rounded-3xl shadow-2xl relative overflow-hidden"
 >
 {/* Background Pattern */}
 <div className="absolute top-0 right-0 -mt-10 -mr-10 w-64 h-64 bg-primary/20 rounded-full blur-3xl" />

 <div className="relative z-10">
 <h2 className="text-lg font-medium text-faint-blue mb-1">경제적 자유 목표액 (FIRE)</h2>
 <div className="flex items-baseline gap-2 mb-8">
 <span className="text-4xl md:text-5xl font-black tracking-tight">{formatNumber(targetAmount)}</span>
 <span className="text-xl font-medium text-faint-blue">원</span>
 </div>

 <div className="space-y-2 mb-8">
 <div className="flex justify-between text-sm font-medium">
 <span className="text-[rgba(255,255,255,0.8)]">진행률</span>
 ...
 <h3 className="text-sm font-medium text-faint-blue mb-2">예상 탈출 시점</h3>
 {yearsToTarget === 0 ? (
 ...
 <span className="text-4xl font-bold text-navy">{yearsToTarget.toFixed(1)}</span>
 <span className="text-xl text-faint-blue">년 후</span>
 <span className="text-sm text-faint-blue ml-2">({new Date().getFullYear() + Math.floor(yearsToTarget)}년)</span>
 </div>
 ) : (
 ...
 {formatNumber(metric.value)} <span className="text-sm font-normal text-faint-blue">{metric.unit}</span>
 </p>
 </motion.div>
 ))}
 </div>
