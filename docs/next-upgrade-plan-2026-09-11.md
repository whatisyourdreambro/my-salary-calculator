...
| **9/28(월) 00:00 KST 이후** | **L10' 회사 description 재빌드**(9/25 추가, 승인⑧·A7'·A4'·COMP-06) — 새 description·대상 164곳 수정일 승격은 빌드 시점 KST 날짜로 켜진다(`src/config/siteDates.ts` `COMPANY_META_DISCLOSED_DATE`). 그 전 빌드는 종전 출력 그대로라 브랜치가 먼저 합쳐져도 무해 | 빌드+콘솔 | ① 9/28 이후 main 푸시 또는 CF Pages 'Retry deployment' ② **운영자 Purge**(Retry 로 재빌드했으면 수동) ③ IndexNow 는 postbuild 가 sitemap lastmod 차이로 대상 164 URL 만 자동 제출 — CF 빌드 로그 `[indexnow]` 확인 ④ 확인: 운영 `/salary-db/krafton` meta·og description 끝이 `공시 평균연봉 1억 2,900만원(2025 사업연도).`, `/salary-db/samsung-electronics` 는 `로그인 없이` 있고 `업데이트` 0건, `/sitemap.xml` krafton lastmod 2026-09-28, `/rss-companies.xml` 첫 item pubDate `Mon, 28 Sep 2026` ⑤ **10/5 서치어드바이저 RSS 2종 제출**(rss.xml·rss-companies.xml) ⑥ 판정 GSC 28일 국가=KR(D+28 = 10/26). 첫 빌드가 10/5 뒤로 밀리면 상수를 실제 배포일로 바꾼 뒤 배포 |
...
| S2-4 | ~~회사 title·description~~ → **L10'(승인⑧) 실행 시 흡수** | 회사 title 은 2026-07-06 결정·"절대 불변". description 은 승인⑧(공시 평균 후미 삽입, 9/28 이후)이 같은 430쪽 대기 | L10' 커밋에 'description 150자 상한·핵심 숫자 선두' 규칙 동봉. title 무접촉. 판정 28일 KR 필터. ★9/25 L10' 구현 완료(150자 상한·자체 추정치 수치 선두 유지·공시 평균 후미) — 9/28 KST 이후 빌드에서 켜짐(§1 9/28 행) | — |
