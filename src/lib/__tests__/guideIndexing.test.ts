...
describe("가이드 검색 응답의 색인 범위", () => {
  it.each(["Mozilla/5.0", "Googlebot/2.1", "Yeti/1.1", "Bingbot/2.0"])("방문자와 허용된 검색 봇에 같은 noindex 헤더를 보낸다: %s", async (ua) => {
  ...
  ])("일반 허브·빈 검색·고유 글에는 noindex를 추가하지 않는다: %s", async (path) => {
    const response = await middleware(request(path, "Googlebot/2.1"));
    expect(response.status).toBe(200);
    expect(response.headers.get("x-robots-tag")).toBeNull();
    expect(response.headers.get("cache-control")).toBeNull();
  });

  it("기존 차단 봇 정책을 유지한다", async () => {
    const response = await middleware(request("/guides?q=tax", "SemrushBot"));
    expect(response.status).toBe(403);
    expect(response.headers.get("cache-control")).toBe("no-store");
  });

  it("기존 canonical 호스트 301은 검색어를 보존한다", async () => {
    const response = await middleware(request("/guides?q=tax", "Googlebot/2.1", "moneysalary.com"));
    expect(response.status).toBe(301);
    expect(response.headers.get("location")).toBe("https://www.moneysalary.com/guides?q=tax");
  });

  it("검색어가 들어가지 않는 허브 canonical과 한영 상호 alternates를 보존한다", async () => {
  ...
  it("Google·네이버의 robots 크롤 접근을 유지해 검색 URL의 noindex를 읽을 수 있다", async () => {
