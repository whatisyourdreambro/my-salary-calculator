...
describe("실수령액 엔진 (calculator.ts 단일 코어)", () => {
  ...
  it("자녀 2명 = 자녀 0명 대비 간이세액표 비고3 공제(월 45,830원)만큼 소득세 감소", () => {
    const c0 = calculateNetSalary(60_000_000, 0, 1, 0, adv);
    const c2 = calculateNetSalary(60_000_000, 0, 1, 2, adv);
    // 소득세법 시행령 별표2 비고3: 8~20세 자녀 2명 45,830원 (연 55만원 자녀세액공제 ÷ 12 절사)
    expect(c0.incomeTax - c2.incomeTax).toBe(45_830);
    // 지방소득세는 소득세의 10% — 월 4,583원
    expect(c0.localTax - c2.localTax).toBe(4_583);
  });
...
describe("/table/2026 표 데이터 (generateData2026 — 정식 엔진 통일)", () => {
  ...
  it("golden 행 값 (상세 페이지와 동일 엔진·식대 20만 기준 — 2026-08-30 통일)", () => {
    ...
    // 2026-09-25 A17: 월 소득세 = 근로소득 간이세액표(별표2) 금액
    //   3,000만 2,233,220 → 2,244,440 (소득세 29,160원 = 표 2,300~2,310천원 1인)
    //   5,000만 3,521,236 → 3,571,546 (소득세 190,620원 = 표 3,960~3,980천원 1인)
    //   1억     6,480,443 → 6,530,913 (소득세 986,720원 = 표 8,120~8,140천원 1인)
    expect(at(30_000_000).monthlyNet).toBe(2_244_440);
    expect(at(50_000_000).monthlyNet).toBe(3_571_546);
