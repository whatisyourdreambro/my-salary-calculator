...
export default function TipsPage() {
 ...
 <h1 className="text-4xl font-black tracking-tight text-navy mb-4 leading-tight">
 직장인 꿀팁 완전정복
 </h1>
 ...
 <Link key={link.href} href={link.href} className="group p-4 border border-canvas rounded-xl hover:border-primary hover:bg-primary/5 transition-all">
 <p className="font-bold text-navy group-hover:text-primary text-sm mb-1 transition-colors">{link.label}</p>
 <p className="text-xs text-faint-blue">{link.desc}</p>
 <ChevronRight size={14} className="text-faint-blue group-hover:text-primary mt-2 transition-colors" />
 </Link>
 ))}
 </div>
 ...
 <div key={ii} className="p-6 rounded-xl border border-canvas bg-white hover:border-primary/30 hover:shadow-sm transition-all">
 <h3 className="font-bold text-navy text-base mb-2 leading-snug">{item.title}</h3>
 <p className="text-muted-blue text-sm leading-relaxed">{item.desc}</p>
 </div>
 ))}
 </div>
 {si === 1 && (
