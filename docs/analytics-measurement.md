...
| 가이드 | `^/guides/` | |
| 홈 | `^/$` | |
| 회사 DB 허브(2026-09-26 추가, RPM-01 ⑦) | `^/salary-db$` | 허브만 따로 — 회사 DB 행(`^/salary-db/`)에는 끝 슬래시가 없어 포함되지 않는다 |
| 회사 순위표(추가) | `^/salary-db/ranking` | 회사 DB 행의 부분집합 — 판정 규칙은 회사 DB 행으로만 본다 |
| 직업 허브(추가) | `^/job$` | 개별 직업 `/job/*` 은 제외 |
| 업계(추가) | `^/industry/` | 애드센스 URL 채널 `/industry/`(9/26 추가) |
| 성과급 모음(추가) | `^/calc/bonus-calculators$` | 계산기 행의 부분집합, URL 채널(9/26 추가) |
| 봉급표 2026 4종(추가) | `^/(teacher\|police\|firefighter\|civil-servant)-pay-2026$` | URL 채널 4개(9/26 추가). 10/14 봉급표 예약 병합 전 기준선 |
| 공무원 2027(추가) | `^/civil-servant-pay-2027$` | B20 페이지 |
| 연봉표(추가) | `^/table/` | URL 채널 `/table/`(9/26 추가) |
| 전체 | 필터 없음 | 가설의 25% 확인용 |
...
### 콘솔 변경 기록표 (RPM-01 · 2026-09-26 — 소급되지 않는 설정이라 날짜가 판정 창의 시작이다)

| 항목 | 어디 | 상태·날짜 | 누가 | 걸리는 판정 |
|---|---|---|---|---|
| GA4 맞춤 측정기준 `nav_type`(이벤트 범위) 등록 | GA4 관리 → 맞춤 정의 | ☐ 등록일 기입(9/24 관리 화면 스냅숏에는 없었다 — 등록이 늦으면 10/2 창이 9/27~10/1 로 줄고 500 미만 규칙으로 10/9 로 밀릴 수 있다) | 운영자 | 10/2·10/9 `nav_type` 탐색 |
| GA4 이벤트 데이터 보관 2개월 → 14개월 | GA4 관리 → 데이터 보관 | ☐ 변경일 기입(= 콘솔팩 C09) | 운영자 | 2개월이면 10/12 전 기준선이 12월 중순에 탐색에서 사라져 1월 판독 불가 |
| 애드센스 URL 채널 10개 추가 | 애드센스 → 보고서 → URL 채널 | ✅ **2026-09-26**(운영자 승인 18번) — 교사·경찰·소방·공무원 2026 봉급표, `/industry/`, `/monthly/`, `/salary/`, `/table/`, `/calc/bonus-calculators`, `/salary-db/compare/` | Claude | 10/12 시험·10/14 봉급표 병합·A32. 이날 이전 값은 없다 |
| GA4 맞춤 측정기준 `dest_tpl`(이벤트 범위) 등록 | GA4 관리 → 맞춤 정의 | ☐ **즉시**(배포 9/26 13:10, 맞춤 측정기준은 소급 안 됨) 등록일 기입(아래 '내비 표면' 절) | 운영자 | 10/19 모듈별 목적지 판독 |
| `autoads_seen` 측정기준·측정항목 등록 | GA4 관리 → 맞춤 정의 | ☐ **즉시**(배포 9/26 13:10, 소급 안 됨)(아래 '자동광고 관측' 절 'GA4 등록') | 운영자 | 10/2·10/9 판독 |

### RPM-03 전면광고 판독 — 10/2·10/9 (수동, 이벤트 추가 없음)

운영자 9/26 답: '전면광고 표시' 계측 이벤트는 **미승인**(승인 5번). 대신 `nav_type` 판독과 같은 날 아래 세 줄을 적는다. 광고는 절대 클릭하지 않는다.

- **(a) 세션 안 문서 로드의 상한**: 애드센스 PV − GA4 `session_start`(같은 창·같은 날짜 설정), 사이트 전체와 `/salary-db` 각각. 같은 날의 애드센스 광고 형식 '모바일 전면(vignette)' 노출과 나란히 적는다. 전면 노출이 이 상한보다 훨씬 많으면, 전면 대부분은 새 문서 로드 없이(= 사이트 안 링크 클릭에서) 떴다는 뜻이다.
- **(b) 템플릿별 세션 안 실제 로드**: GA4 `page_view`(`nav_type=landing`) − 방문 수(entrances), 위 10/2 표의 템플릿마다. 새로고침·전면광고 뒤 로드·일반 `<a>` 링크가 여기에 잡힌다.
- **(c) 수동 확인 1회**: 운영 사이트를 개발자 도구 모바일 에뮬레이션으로 열고 회사 페이지에서 본문 Next Link 하나를 누른다 → 주소에 `#google_vignette` 가 붙는지, 다음 문서가 새로 로드되는지(네트워크 탭의 document 요청) 본다. **보기만 하고 광고는 누르지 않는다.**
- **결정 규칙**: 사이트 안 클릭에서 전면이 뜨면 내부 클릭 1회의 가치를 `P(전면) × 3.1원 + 0.26원` 으로 두고, 수익 추천 #1 의 '전체 로드' 이득을 앵커(1,000 PV 당 $1.18)와 자동 인페이지(1,000 PV 당 $0.75~1.25)만으로 다시 적는다. 뜨지 않으면 추천 #1 가정을 그대로 둔다. (a)~(c) 로도 결론이 안 나면 이벤트 없이 자료를 붙여 운영자에게 한 번 더 묻는다.

| 조회일 | (a) 애드센스 PV − GA4 session_start (전체 · /salary-db) | 같은 창 전면 노출 | (b) landing page_view − 방문 수(템플릿별 요약) | (c) `#google_vignette` · 새 문서 로드 | 결론 |
|---|---|---|---|---|---|
| 10/2 | | | | | |
| 10/9 | | | | | |
...
## L10' 판정 — GA4 네이버 유입 비교 (2026-09-26 운영자 승인 9번 · NAVER-02, 10/30 조회)

**왜 GSC 가 아닌가**: GSC 국가=KR 28일 페이지 목록에는 클릭이 있는 회사 페이지가 없다(9/24 추출). 계획대로 GSC 로 보면 빈 판정이 나와 '효과 없음'으로 잘못 읽힌다. 회사 페이지 세션은 85~90% 가 네이버다.

- **처치·대조**: 처치 = L10' 꼬리(공시 평균연봉)가 붙은 164곳 — 10/1 빌드 뒤 `/sitemap.xml` 에서 lastmod 가 2026-10-01 로 오른 `/salary-db/*` URL 로 목록을 뽑는다. 대조 = 나머지 266곳. 두 군 모두 COMP-06/A7' 문구 변경은 함께 받았다.
- **창**: 전 9/3~9/30 · 후 10/2~10/29(완료일만, 10/1 켜짐 당일 제외). **조회 10/30.**
- **GA4 탐색(자유 형식)**: 측정기준 `방문 페이지 + 쿼리 문자열`(랜딩 페이지)·`세션 소스` · 측정항목 `세션수` · 필터 ① `세션 소스` 에 `naver` 포함(`m.search.naver.com`·`search.naver.com`·`naver` 등 모두) ② `방문 페이지` 정규식 `^/salary-db/[^/]+$`. 비교 기간을 켜서 두 창을 한 표로 받고 CSV 로 내보낸다(원본은 저장소 밖).
- **계산**: 회사마다 `(후 세션 − 전 세션) ÷ 전 세션`. 군별 **중앙값**을 비교한다(합계는 대형사 몇 곳이 좌우한다). 전 세션이 10 미만인 회사는 비율 대신 절대 증감만 따로 적는다.
- **빼는 회사**: ① 뉴스 급등 — 두 창 중 하루 세션이 그 회사 창 일평균의 5배 이상인 날이 있는 회사(제안 기준, 판정 전 운영자 확인) ② 별칭이 바뀐 회사(W3-F) — 검색어 매칭이 달라져 L10' 효과와 섞인다. 뺀 회사 목록과 사유를 표 아래에 적는다.
- **보조 자료**: 네이버 서치어드바이저 주간 상위 30(9/28 부터 매주 월요일, 콘솔팩)에 잡히는 처치군 페이지(lig-nex1·korea-zinc·cosmax·doosan-enerbility·gs-caltex 등)의 클릭·노출 시계열.
- **읽는 법**: 처치 중앙값이 대조보다 뚜렷이 낮으면 '나쁨'(차이 기준은 10/30 전에 운영자와 확정) — 회사 설명문 2차(승인 6번·NAVER-04, 10/10 에 꺼진 채 합류)를 켜지 않는다. 그 밖에는 2차를 켠다. 네이버 순위·검색어 구성 변화가 함께 움직이므로 세션 증감 하나로 제목·설명 성패를 단정하지 않는다.

| 조회일 | 처치 회사 수(제외 후) | 대조 회사 수(제외 후) | 처치 중앙값 증감 | 대조 중앙값 증감 | 제외 회사·사유 | 판정 |
|---|---|---|---|---|---|---|
| 10/30 | | | | | | |
