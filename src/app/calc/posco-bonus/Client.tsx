...
export default function PoscoBonusClient() {
  ...
  const [customPercent, setCustomPercent] = useState(400);
  ...
  const calc = useMemo(() => {
    ...
    return {
      ...
      allScenarios: SCENARIOS.map((s) => ({ label: s.label, gross: monthlyBasicWon * (s.percent / 100) })),
    };
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-canvas-deep bg-white p-6 sm:p-8">
        ...
        <p className="mt-2 text-xs text-faint">추정 연봉(기본급 × 18) = <strong>{fmtEok(calc.estimatedAnnualSalary)}</strong></p>
      </section>
      ...
      <section className="rounded-2xl border-2 border-primary bg-gradient-to-br from-primary/5 to-primary/10 p-6 sm:p-8">
        ...
        <div className="rounded-xl bg-white border border-primary/30 p-5">
          ...
          <p className="text-3xl sm:text-4xl font-black text-primary tabular-nums">{fmtEok(calc.bonusWon)}</p>
