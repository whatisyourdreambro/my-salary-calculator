...
// ★ 2027 전체 호봉표는 싣지 않는다 — 저연차 추가 인상·국회 조정 전이라 오정보. 대표 호봉 예상치만(payForecast2027.ts).
// ★ 중복 제목 방지(네이버): 2026 페이지의 연도만 바꾼 문구 금지 — 일정·2026 대비 차액·수당 문단을 새로 쓴다.
// ★ 갱신 체크포인트: 12월 말 2027 봉급표 공표 → 확정표 체제로 전환(teacher-pay-2026 구조 + datasetLd),
//   '예상' 표기·예산안 문구 정리, sitemap lastModified 갱신.
...
const PAGE_TITLE = `2027 교사 봉급표 예상 — 예산안 ${pct}%·교원 호봉별 월급`;
const PAGE_DESCRIPTION = `2027년 교사 봉급 인상률은 정부 예산안 ${pct}%로 국회 심의 중(확정 전)입니다. 2026년 교원 봉급표에 적용한 ${START.label} 약 ${fmt(START.predicted2027)}원 등 주요 호봉 예상 월급과 확정 일정을 정리했습니다.`;
...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    answer: `2026년 9호봉 월 봉급 ${fmt(START.base2026)}원에 ${pct}%를 단순 적용하면 약 ${fmt(START.predicted2027)}원으로 월 ${fmt(START.monthlyIncrease)}원가량 늘어납니다(천원 단위 반올림, 확정 봉급표 아님). 2026년에는 공통 인상률 3.5%와 별도로 저연차 구간이 더 올라 9호봉이 5.5% 인상됐으므로, 2027년에도 저연차 조정이 있으면 실제 금액은 이보다 클 수 있습니다. 담임수당·정액급식비 등 수당은 봉급과 별도입니다.`,
  },
  {
...
const SCHEDULE = [
  ...
  },
  {
    ...
    desc: `기획예산처가 2027년도 예산안에 공무원 보수 ${pct}% 인상을 반영했다고 보도됐습니다(2011년 5.1% 이후 최대). 정부안이며, 교원 봉급표의 호봉별 금액은 아직 정해지지 않았습니다.`,
    done: true,
  },
  {
    step: "3",
      ...
      <div className="page-width">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl sm:text-5xl font-black tracking-tight text-navy mb-4">
            2027 교사 봉급표 <span className="text-electric">예상 월급</span>
          </h1>
          ...
          <p className="text-base sm:text-lg text-muted-blue leading-relaxed max-w-2xl mx-auto">
            ...
            {fmt(START.predicted2027)}원(단순 예상치)입니다.
          </p>
          ...
          <div className="min-w-0 max-w-3xl mx-auto lg:mx-0 w-full">
            ...
            <section className="mt-10 mb-12 p-6 sm:p-8 bg-white rounded-3xl border border-canvas-200">
              ...
              <PayForecastTable
                ...
                rows={ROWS}
                firstColumn="호봉"
                regionLabel="2027 교원 봉급 예상표 (가로 스크롤)"
              />
            ...
            <section className="mt-10 mb-12">
              <h2 className="text-lg font-black text-navy mb-4">2027 교원 봉급 확정까지 — 4단계 일정</h2>
              ...
              <h2 className="text-lg font-black text-navy mt-10 mb-3">수당은 {pct}%와 따로 움직인다</h2>
              <ul className="text-sm leading-7 text-muted-blue list-disc pl-5">
                ...
                <li>
                  ...
                  {fmt(holidayBonus(START.base2026))}원 → 2027년 약 {fmt(holidayBonus(START.predicted2027))}원(예상).
                </li>
              </ul>
            </section>
            ...
            <section className="mb-12 grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              >
                ...
                <p className="text-sm opacity-90">지금 받는 1~40호봉 확정 금액</p>
              </Link>
              <Link
            ...
            <div className="mt-8">
              <ShareButtons
                title={`2027 교사 봉급표 예상 — 예산안 ${pct}% 적용 호봉별 월급`}
                description={`9호봉 약 ${fmt(START.predicted2027)}원(예상). 확정 전 단순 계산이며 수당·저연차 추가 인상은 미반영입니다.`}
              />
            </div>
          </div>
