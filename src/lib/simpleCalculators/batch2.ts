   ...
   faqs: [
     {
       ...
       a: "이직 후 거주지 관할 고용센터를 방문하거나 고용24(work24.go.kr)에서 온라인으로 수급자격 인정을 신청하고, 인정 뒤에는 2~4주마다 구직활동을 보고하는 실업인정을 거쳐 지급받습니다. 고용보험법 제48조에 따라 이직일 다음 날부터 12개월이 지나면 남은 일수와 관계없이 지급이 끝나므로 이직 후 서둘러 신청하는 것이 유리합니다.",
     },
     {
       q: "자진 퇴사하면 실업급여를 못 받나요?",
       a: "원칙적으로 자발적 이직은 구직급여 대상이 아닙니다. 다만 임금 체불, 직장 내 괴롭힘, 채용 시 제시된 근로조건과 실제 조건의 현저한 차이, 질병·부상으로 업무 수행이 어려운 경우처럼 고용보험법령이 정한 정당한 이직 사유에 해당하면 자진 퇴사라도 수급이 가능하며, 사유 해당 여부는 고용센터가 증빙을 보고 심사합니다.",
     },
     {
       q: "실업급여 받는 동안 아르바이트를 할 수 있나요?",
       a: "수급 중 소득이 생기는 일을 하면 실업인정 때 반드시 신고해야 합니다. 신고한 근로일은 그날의 구직급여가 지급되지 않거나 조정되고, 신고 없이 일하다 적발되면 부정수급으로 받은 급여 반환에 추가 징수까지 될 수 있습니다. 주 15시간 이상 계속 근로처럼 취업으로 보는 경우에는 수급이 중단됩니다.",
     },
     {
       q: "실업급여는 한 달에 얼마나 받나요?",
       a: "구직급여는 1일 단위로 계산해 실업인정일마다 지급하므로 월 금액은 일액에 그 기간의 일수를 곱한 값입니다. 2026년 이직자는 일액이 하한 66,048원에서 상한 68,100원 사이로 정해져 평균 월급이 높아도 상한을 넘지 못하고, 이 계산기의 기본값(평균 월급 300만원)에서는 하한이 적용되어 30일 기준 월 환산 수급액이 1,981,440원입니다.",
     },
   ],
   caveats: [
     "2026년 이직자 기준 상한 68,100원·하한 66,048원을 적용하며, 이직일이 2025년 이전이면 그 해의 상·하한이 적용되어 결과가 달라집니다.",
     "소정급여일수는 50세 미만 기준 120~240일만 반영하고, 50세 이상·장애인의 구간별 30일 가산(최대 270일)은 넣지 않았습니다.",
     "이직 전 18개월 중 피보험 단위기간 180일 이상, 비자발적 이직 등 수급 요건 충족 여부는 판단하지 않으며 자발적 이직은 원칙적으로 대상이 아닙니다.",
     "평균임금은 세전 기본급과 고정수당 기준의 어림값이며, 상여·연차수당 포함 여부와 일할 계산은 고용센터 산정과 다를 수 있습니다.",
   ],
   relatedSlugs: ["severance-pay-quick", "annual-leave-pay-quick", "earned-income-tax-quick"],
   details:
     "고용보험 구직급여(실업급여)를 이직 전 3개월 평균 월급과 근무 기간만으로 어림하는 계산기입니다. 비자발적 이직을 앞두고 받을 금액의 규모를 먼저 보려는 근로자를 위한 것이며, 기본값은 평균 월급 300만원, 근무 24개월인 50세 미만 근로자의 경우입니다.\n\n기본값에서는 평균 월급 3,000,000원을 30일로 나눈 1일 평균임금의 60%가 2026년 하한액에 못 미쳐 일일 구직급여가 하한액인 66,048원으로 정해지고, 근무 24개월은 소정급여일수 150일 구간이라 예상 총 수급액은 9,907,200원, 월 환산 수급액은 1,981,440원입니다. 평균 월급이 더 높아 60% 금액이 상한을 넘으면 일액은 68,100원에서 멈춥니다.\n\n고용보험법 제46조(2026년 8월 20일 시행 기준)는 구직급여일액을 기초일액의 60%로 하되 제45조 제4항에 따른 경우 기초일액의 80%를 최저구직급여일액으로 두고 60% 금액이 그보다 낮으면 최저구직급여일액을 지급하도록 정하며, 제40조는 기준기간의 피보험 단위기간 합산 180일 이상을 수급 요건으로, 제48조는 이직일 다음 날부터 12개월 안에서 소정급여일수를 한도로 지급하도록 정합니다. 고용노동부는 2025년 12월 16일 보도자료에서 2026년 최저임금 인상으로 하한액이 상한액을 넘게 되어 기초일액 상한을 11만원에서 11만3,500원으로 올리고 구직급여 상한액을 66,000원에서 68,100원으로 인상한다고 밝혔습니다. 하한 66,048원은 2026년 최저임금 시급 10,320원(고용노동부 고시)의 80%에 1일 8시간을 곱한 값입니다.\n\n50세 이상·장애인의 소정급여일수 가산, 일용·예술인·노무제공자 특례, 자발적 이직의 예외 사유, 대기기간과 실업인정, 조기재취업수당은 계산하지 않습니다. 실제 수급 자격과 일수는 고용24(고용센터)의 수급자격 인정으로 정해지므로, 나이별 수급 기간과 상한 적용은 실업급여 계산기 상세 페이지에서, 퇴직금은 퇴직금 간편 계산에서 이어서 확인하세요.",
   sources: [
     { title: "고용노동부 — 고용보험법 시행령 개정안 국무회의 의결 보도자료, 구직급여 상한액 68,100원(2026)", url: "https://www.moel.go.kr/news/enews/report/enewsView.do?news_seq=18736" },
     { title: "법제처 국가법령정보센터 — 고용보험법 제46조 구직급여일액(2026)", url: "https://www.law.go.kr/법령/고용보험법/제46조" },
   ],
 },
];
