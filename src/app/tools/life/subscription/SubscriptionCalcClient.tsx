    ...
    <main className="w-full min-h-screen bg-white px-4 pt-28 pb-20 font-sans">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-12 pb-10 border-b border-gray-100">
          ...
          <h1 className="text-3xl font-black text-slate-900 mb-3">구독 서비스 지출 계산기</h1>
          <p className="text-slate-500 font-medium">매월 나도 모르게 빠져나가는 구독료를 한눈에 파악하세요.</p>
        </div>

        <AdUnit slotId="1122998877" format="auto" label="구독 상단 광고" />

        <div className="grid md:grid-cols-2 gap-8 mt-8">
          {/* Left: Add Section */}
          <div className="space-y-6">
            <div className="border border-gray-200 rounded-2xl p-6">
              <h2 className="font-black text-slate-900 mb-4 flex items-center gap-2"><PlusCircle size={18} className="text-primary" /> 빠른 추가</h2>
            ...
            <div className="border border-gray-200 rounded-2xl p-6">
              <h2 className="font-black text-slate-900 mb-4">직접 입력</h2>
              <div className="space-y-3">
                <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="서비스명" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none" />
                <input value={newAmount} onChange={e => setNewAmount(e.target.value)} type="number" placeholder="금액 (원)" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none" />
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => setNewPeriod("monthly")} className={`py-2 rounded-xl text-xs font-bold border transition-all ${newPeriod === "monthly" ? "bg-primary text-white border-primary" : "border-gray-200 text-slate-600"}`}>월간</button>
                  <button onClick={() => setNewPeriod("yearly")} className={`py-2 rounded-xl text-xs font-bold border transition-all ${newPeriod === "yearly" ? "bg-primary text-white border-primary" : "border-gray-200 text-slate-600"}`}>연간</button>
                </div>
          ...
          <div className="space-y-6">
            ...
            <div className="border border-gray-200 rounded-2xl overflow-hidden">
              {subs.length === 0 ? (
                <div className="p-8 text-center text-slate-400 text-sm">구독 서비스를 추가해보세요</div>
              ) : (
                subs.map((s, i) => (
        ...
        <div className="mt-8">
          <AdUnit slotId="2233001122" format="auto" label="구독 하단 광고" />
        </div>
      </div>
    </main>
  );
