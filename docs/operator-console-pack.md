...
6. ☐ **10/5 네이버 RSS 2종 상태 확인**(★9/26 정정 — 제출 아님): 두 피드는 **8/15 부터 등록돼 있다**(서치어드바이저 제출 화면 등록 시각 8/15 23:17, `docs/revenue-audit-2026-09-08/live-naver-audit.md`). 세션 4 ① 에서 마지막 수집 시각·오류만 본다 — 같은 주소를 다시 제출하지 않는다. 확인 전 `rss-companies.xml` 에 삼성전자·SK하이닉스·현대차가 있는지 본다(감사 통합 배포 뒤 430곳 전부, B7).
...
## ★2026-09-26 네이버·GSC 통합 체크리스트 — 날짜순 (운영자 9/26 CONSOLE-NAVER · 승인 원장 `docs/next-upgrade-plan-2026-09-11.md` §10)

한 번에 25분을 넘기지 않는다. 원본 CSV·캡처는 저장소 밖, 문서에는 집계 한 줄만.

**상시 규칙**
- **바뀌지 않은 URL 은 다시 요청하지 않는다**(네이버 수집 요청·GSC 색인 요청 모두). 요청은 그 배포에서 실제로 바뀐 URL 만 — 목록은 CF 빌드 로그 `[indexnow]` 가 보낸 URL 과 같아야 한다.
- **재빌드 규칙(CRAWL-01 이후 — CRAWL-01 은 9/26 현재 상정 중, 적용되는 날부터)**: A35 자동 Purge 가 2번 성공한 뒤 HTML 엣지 캐시를 1시간 → 24시간으로 늘리면, 예약 재빌드(10/1·11/25·12/1·12/16)는 CF Pages 'Retry deployment' 대신 **빈 커밋 main 푸시**로 해야 `cf-purge.yml` 이 돈다. Retry 로 재빌드했다면 **수동 Purge Everything** 을 바로 한다. 안 하면 24시간 동안 옛 HTML 이 나간다.

| 날짜 | 할 일 | 계획 ID | 조건·주의 |
|---|---|---|---|
| **9/28(월)부터 매주 월요일 5분** | 서치어드바이저 → 리포트 → 검색 유입 → 최근 7일 **상위 30 페이지·상위 30 검색어** 캡처 또는 내보내기 → `docs/metrics-log.md` 에 한 줄(날짜·상위 5 페이지 클릭·특이 변화). 변경마다 시계열을 남기는 용도 | NAVER-02 | 공변량 메모: 9/25 /job 억 표기(B14)·9/29 파비콘·10/1 L10'·10/14 봉급표. 네이버 자료는 약 1주 늦게 갱신된다 |
| 9/29 배포 뒤(파비콘) | 서치어드바이저 → 요청 → 웹 페이지 수집 → `/` 1건과 상위 몇 쪽 **수집 요청** | NAVER-01 | 파비콘 파일 URL 은 그대로다(바꾸지 않음). 7~14일 뒤 네이버 모바일·통합검색 결과의 파비콘이 파란 사각형이 아닌지 확인 |
| 10/2 부터(호스트가 7일 연속 깨끗할 때) | GSC → 색인 생성 → 페이지 → **서버 오류(5xx) → 수정 결과 확인** 시작(51행, 전부 7/9~8/6 크롤 — 8월 /salary 500 사건) | CRAWL-10 | 조건: 크롤링 통계 호스트 상태 '서버 연결' 7일 연속 양호(C16). 아니면 미룬다 |
| A28 T0+7 뒤 | GSC **찾을 수 없음(404) → 수정 결과 확인**(사이트맵 범위 102행 = 용어집·QnA, 오늘은 200) | CRAWL-10 | '-N' 꼬리 143개 404 는 그대로 둔다(리다이렉트 불필요). `-manwon` 31개는 이미 308 |
| 10/2 (C15 ② 분류 뒤) | 네이버 수집 오류 목록을 HTML / OG·API 로 나눈 뒤, **실패한 수익 HTML 쪽만** 수집 요청(예: `/salary-db/kakao-ent`·`/salary-db/mckinsey-korea`) | CRAWL-10 | OG·API 는 요청하지 않는다 |
| **10/5 세션 4 칸** | ① RSS 2종 **상태 확인**(8/15 등록 — 제출 아님) ② **C15 ⑤ 중복 제목 그룹 분류**: 사이트 진단 '제목 중복'(559) 상세에서 그룹 제목·개수를 적는다 — `/guides?q` 변형 비중, `/?tab=` 홈 변형(rank·freelancer·severance·future·exchange), utm 공유 링크(`?utm_source=copy\|kakao&utm_medium=share`), 아펙스 호스트. 결과표는 아래 ③ **/home-loan 네이버 웹 탭 순위** — '주담대 금리 계산기'·'주택담보대출 계산기'·'대출한도 계산' 각각 우리 순위(10위 안이면 10/6~10/7 제목·설명 1회 교정 배포, 모두 밖이면 건너뜀) ④ 저CTR 검색어 10개 SERP 캡처(AI 브리핑 유무·인용 도메인·웹 탭 순위·파비콘): 2027년 공무원 봉급표 · 2027 공무원 봉급표 · 2027년 공무원 인상률 · 주담대 금리 계산기 · 현대차 성과급 · 코트라 연봉 · lig넥스원 연봉 · 삼성전자 연봉 · 직업군인 연봉 · 교사 호봉표 | NAVER-08(b)·NAVER-06·NAVER-02(d) | ②가 여전히 `?q` 변형 위주면 가이드 태그 링크 `#q=` 전환(W3-E, `claude/r3-w3e-20260926`)을 10/12 에 배포, 아니면 보류하고 다른 계열부터 본다. `nosourceinfo` 로봇 태그는 절대 넣지 않는다(AI 브리핑 인용 자격 유지) |
| **10/12~10/13 (10분)** | 시즌 URL 약 18개(연말정산 허브·공제 4종·연말정산 계산기·시즌 가이드 등 — 목록은 세션 직전 Claude 가 사이트맵·시즌 링크 세트에서 뽑아 둔다)의 네이버 색인 상태 확인(`site:` 검색 또는 서치어드바이저) → **색인 안 된 URL 만** 수집 요청 | SEASON-13 | 시즌 배포(10/14~10/31)와 동결기 상수 교체 때마다 바뀐 URL 만 수집 요청 + CF 빌드 로그 `[indexnow]` 확인 + 시즌 HTML 의 `CF-Cache-Status: HIT` 표본 확인, 단계 재빌드 뒤 Purge |
| 가이드 배포일마다 (10분) | 재작성된 대표 글(배치당 약 12~14개) **네이버 웹 페이지 수집 요청** + 상위 10개 GSC URL 검사 → CF 빌드 로그 `[indexnow]` 가 lastmod 바뀐 URL 만 보냈는지 확인 → 배포 시각을 `docs/ad-experiments.md` 3절 (c) 공변량 표에(URL 채널 `/guides`) | GUIDES-12 | 308 통합된 옛 주소는 요청하지 않는다. D+28 판독은 `docs/gsc-sniping-log.md` 새 Round |

**10/5 C15 ⑤ 중복 제목 그룹 기록표**

| 그룹 제목(앞 20자) | URL 수 | 계열(`?q` · `?tab=` · utm · 아펙스 · 기타) | 대표 URL | 메모 |
|---|---|---|---|---|
| | | | | |
...
## 세션 4 — 10/5 (25분)
1. ☐ **네이버 서치어드바이저** (10분): searchadvisor.naver.com → 웹마스터 도구 → moneysalary.com → 요청 → **RSS 제출 화면에서 상태만 확인**(★9/26 정정: `rss.xml`·`rss-companies.xml` 은 8/15 부터 등록됨 — 다시 제출하지 않고 마지막 수집 시각·오류만 캡처) → 리포트 → 검색 유입(검색어·페이지 최근 28일)·콘텐츠 수집 현황·사이트 진단 각 캡처(검색어·페이지 **내보내기**는 9/13 세션 2 ①로 앞당김 — 여기서는 RSS 상태 확인 + 재캡처만).
   - **상태 확인 전 점검**: 브라우저에서 `https://www.moneysalary.com/rss.xml` 을 열어 /insights 리포트 3편(성과급 실지급률·상장사 평균연봉 TOP100·업종별 초봉)이 `<category>데이터 리포트</category>` item 3건으로 실려 있는지 본다 — 2026-09-05 코드 합류분(배포 후 최대 1시간 캐시 s-maxage=3600 반영 대기). 피드가 이미 등록돼 있어 제출 버튼은 누르지 않는다.
