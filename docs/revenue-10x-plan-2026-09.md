...
| L14'·M07 연말정산 클러스터 | ✅ 배포(2차) | relatedCalculators `yearEnd` 카테고리 신설(리뷰 발견: tax 끝에 붙이면 limit=4 라 도달 0) — 허브·/year-end-tax·공제 4종·R2 3종만 우선 소비, 다른 tax 페이지 4종 불변(테스트 고정). R2 3종 page 에 클러스터 탭(마지막 광고 아래). YEAR_END_STEPS 중복 1건→dependent-check 치환(16 불변). `src/lib/yearEndSeason.ts` 연도 상수(문자열 바이트 동일 증명) — ~~12/15 값 교체~~ **취소(2026-09-26 R3 W1-A)**: 2026년 귀속을 2027-03-10 까지 유지, 전환은 2027-03-11 부터(`yearEndSeasonGuard.test.ts` 가 12/15 교체를 막음) |
...
| M07 | 연말정산 클러스터 title 연도 프레임 상수화(`YEAR_END_SEASON_LABEL`, '2027 (2026년 귀속)') — ~~값 교체 12/15~~ **12/15 슬롯 취소(2026-09-26 R3 W1-A)**: 2026년 귀속을 2027-03-10 까지 유지, 전환은 2027-03-11 부터(`yearEndSeasonGuard.test.ts`). ★**2026-09-26 운영자 승인 10번(SEASON-06)**: 12/15 값 교체 계획은 폐기하고, 연말정산 약 12쪽의 title 을 신고 연도가 앞에 오는 '2027 연말정산(2026년 귀속)' 형식으로 **10/14~10/16 에 한 번만** 바꾼다(주소 불변, 미리보기 쪽 제외). 12월에 바꾸면 2026년 소득을 정산 중인 사람에게 '2027년 귀속'이 잘못 붙는다 | [코드] | ×1.00~1.02 | L14'와 같은 배치. 1월 초 교체 금지. 제목 변경은 10/14~10/16 1회 뒤 동결 |
