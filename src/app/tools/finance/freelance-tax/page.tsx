...
export default function FreelanceTaxPage() {
 ...
 <h1 className="text-4xl font-black text-navy tracking-tight mb-3">프리랜서 종합소득세 계산기</h1>
 <p className="text-faint-blue font-medium">사업소득·프리랜서 수입의 2026년 종합소득세를 계산합니다</p>
 </div>
 ...
 <div className="text-center"><p className="text-navy/60 text-xs mb-1">순이익(세전)</p><p className="text-navy font-black">{fmt(r.netIncome)}원</p></div>
 ...
 <div className="w-px bg-white/20" />
 <div className="text-center"><p className="text-navy/60 text-xs mb-1">실효세율</p><p className="text-navy font-black">{r.effectiveRate.toFixed(1)}%</p></div>
 </div>
 </div>
 ...
 <span className={`font-black tabular-nums ${item.main ? "text-primary text-xl" : item.value < 0 ? "text-electric" : "text-navy"}`}>{item.value < 0 ? "-" : ""}{fmt(Math.abs(item.value))}원</span>
 </div>
 ))}
 </div>
 </motion.div>
