    ...
    <main className="min-h-screen bg-white pb-24 pt-28 px-4 font-sans">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-12 pb-10 border-b border-gray-100">
          ...
          <h1 className="text-4xl font-black text-slate-900 tracking-tight mb-3">복리 계산기</h1>
          <p className="text-slate-500 font-medium">원금 + 월 적립 + 복리 수익률로 미래 자산을 계산합니다</p>
        </div>

        <div className="bg-white border border-gray-200 rounded-2xl p-8 mb-6 shadow-sm space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-2">초기 투자금 (원)</label>
              <input type="number" value={principal} onChange={e => setPrincipal(Number(e.target.value))}
                className="w-full border border-gray-200 rounded-xl px-4 py-3 font-bold text-slate-900 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none" />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-2">월 적립금 (원)</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-2">연 수익률 ({rate}%)</label>
              ...
              <div className="flex justify-between text-xs text-slate-400 mt-1"><span>1%</span><span className="text-primary font-bold">{rate}%</span><span>20%</span></div>
            </div>
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-2">투자 기간 ({years}년)</label>
              <input type="range" min={1} max={40} value={years} onChange={e => setYears(Number(e.target.value))} className="w-full accent-primary" />
              <div className="flex justify-between text-xs text-slate-400 mt-1"><span>1년</span><span className="text-primary font-bold">{years}년</span><span>40년</span></div>
            </div>
          </div>
        </div>
        ...
        <div className="border border-gray-200 rounded-2xl overflow-hidden mb-8">
          <div className="bg-slate-50 px-6 py-4 border-b border-gray-100 flex justify-between">
            <h3 className="font-black text-slate-900 text-sm">연도별 자산 시뮬레이션</h3>
          </div>
          <div className="divide-y divide-gray-100 max-h-64 overflow-y-auto">
            {r.data.map(d => (
              <div key={d.year} className="flex items-center gap-4 px-6 py-3">
                <span className="text-xs font-bold text-slate-400 w-12">{d.year}년</span>
                ...
                <span className="text-xs font-black text-slate-900 w-28 text-right">{fmt(d.balance)}원</span>
              </div>
            ))}
          </div>
        </div>
        <AdUnit slotId="compound_bottom" format="auto" label="복리 하단" />
      </div>
    </main>
  );
}
